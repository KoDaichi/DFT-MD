...
contains
  ...
  subroutine sfb_angular(sfb, R_ij, R_ik, d_ij, d_ik, cos_ijk, values, &
    ...
    ! fc_j = sfb_fc(d_ij, sfb%a_Rc)  !daichi
    fc_j = sfb_fc(d_ij, sfb%a_Rc, kfc_r, r_h)
    ! fc_k = sfb_fc(d_ik, sfb%a_Rc)  !daichi
    fc_k = sfb_fc(d_ik, sfb%a_Rc, kfc_r, r_h)
    w_ijk = fc_j*fc_k
    ...
    if (present(deriv_i) .and. present(deriv_j) .and. present(deriv_k)) then
       ! dfc_j = sfb_fc_d1(d_ij, sfb%a_Rc)  !daichi
       dfc_j = sfb_fc_d1(d_ij, sfb%a_Rc, kfc_r, r_h)
       ! dfc_k = sfb_fc_d1(d_ik, sfb%a_Rc)  !daichi
       dfc_k = sfb_fc_d1(d_ik, sfb%a_Rc, kfc_r, r_h)
