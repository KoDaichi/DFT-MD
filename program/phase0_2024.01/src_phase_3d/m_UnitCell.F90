...
function tensor_rank2_maxdiff(t1,t2) result(mdiff)
  real(kind=DP), dimension(3,3) :: t1,t2
  real(kind=DP) :: mdiff,val
  integer :: i,j
  mdiff=0.d0
  do i=1,3
     do j=1,3
        val = max(t1(j,i),t2(j,i))
        if(abs((t1(j,i)-t2(j,i))/val).gt.mdiff) then
           mdiff = abs((t1(j,i)-t2(j,i))/val)
        endif
     enddo
  enddo
  return
end function tensor_rank2_maxdiff

function tensor_rank2_maxval(t1) result(mval)
  real(kind=DP), dimension(3,3) :: t1
  real(kind=DP) :: mval
  integer :: i,j
  mval=0.d0
  do i=1,3
     do j=1,3
        if(abs(t1(j,i))>mval) mval = abs(t1(j,i))
     enddo
  enddo
  return
end function tensor_rank2_maxval
  ...
  subroutine first_half()
    ...
    do it=1,nrsv_t ! thermo loop
      ...
      if(control_pressure ==  ON)then
        ...
        enddo
!        write(nfout,'(a,i8,f15.10)') '!** max diff metric',iteration_unit_cell,tensor_rank2_maxdiff(metric, metric_tmp)
        metric = metric_tmp
      endif
    ...
    if(control_pressure == ON)then
      do it=1,nrsv_t
        ...
        do iatm=1,thermostats(it)%natm
          ...
          enddo
        enddo
        p_baro_tmp_m = thermostats(it)%p_baro
        thermostats(it)%p_baro(:,:) = thermostats(it)%p_baro(:,:) - 0.5d0 * dtio * thermostats(it)%s_thermo * &
        & (dudmetric(:,:) + 2.d0 * factor(:,:)*(-p33(:,:)+p_baro33(:,:) +     &
        &  metric_inv(:,:)*(pot_baro - k_baro)))
!        write(nfout,'(a,i8,f15.10)') '!** max diff p_baro',iteration_unit_cell, &
!        tensor_rank2_maxdiff(thermostats(it)%p_baro, p_baro_tmp_m)
!        write(nfout,'(a,i8,f15.10)') '!** max val  p_baro',iteration_unit_cell, &
!        tensor_rank2_maxval(thermostats(it)%p_baro)
      enddo
    endif
