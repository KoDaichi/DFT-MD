...
!=======================================================================
  ...
  if(cnst_typ ==  BONDLENGTH_FIX_1) then
     ia1= ia_cnst(1)
     ia2= ia_cnst(2)
...
!=======================================================================
  ...
  use m_Const_Parameters, only : DP, FIX &
       & ,   BONDLENGTH_FIX_1, BONDLENGTH_FIX_2, COG_FIX_L
  implicit none
  integer, intent(in)      :: natm,ityp(natm),cnst_typ,nfcatm,ia_cnst(nfcatm)
  real(kind=DP),intent(in) :: amion(*),cps(natm,3)
  integer, intent(in)      :: imdtyp(natm)
  real(kind=DP),intent(in) :: sgmc(4)
  real(kind=DP),intent(out):: sigma

  integer       :: ia1,ia2,ia,j,m, nfout, ntyp
  real(kind=DP) :: tmass, rmass, gmass

!!$  nfout = 6
!!$  write(nfout,'("!cnstra natm, nfcatm = ",i8, i8)') natm,nfcatm
!!$  do ia = 1, 2
!!$     write(nfout,'("!cnstra amion(",i4,") = ",f16.8)') ia,amion(ia)
!!$  end do
!!$  do ia = 1, nfcatm
!!$     write(nfout,'("!cnstra ia_cnst(",i4,") = ",i8)') ia,ia_cnst(ia)
!!$  end do
!!$  do ia = 1, natm
!!$     write(nfout,'("!cnstra ia, imdtyp(ia) = ",2i8)') ia, imdtyp(ia)
!!$  end do
...
!!$     write(nfout,'("!cnstra rmass, tmass = ",2f16.8)') rmass,tmass
     rmass = rmass - tmass
     gmass = tmass*rmass /(tmass + rmass)
!!$     write(nfout,'("!cnstra gmass  = ",f16.8)') gmass
!!$     write(nfout,'("!cnstra nfcatm = ",i8)') nfcatm
     sigma= 0.d0
     do ia = 1, natm
!!$        write(nfout,'("!cnstra ia, imdtyp(ia) = ",2i8)') ia, imdtyp(ia)
        if(imdtyp(ia) /= FIX) then
     ...
     write(nfout,'("!cnstra sigma = ",f8.4)') sigma
     sigma = sigma -sgmc(4)
     return
  end if
...
contains
  ...
  integer function operatorExists_in_nop()
    ...
    if(operatorExists_in_nop == -1) then
       ...
       call flush(6)
       call phase_error_with_msg(6,'!Illegal value of iop_opxop(iop,jop)',__LINE__,__FILE__)
    end if
