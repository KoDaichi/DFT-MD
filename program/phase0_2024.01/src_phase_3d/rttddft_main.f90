...
use m_Electronic_Structure,only : &
    m_ES_energy_eigen_values_3D
use m_ES_LHXC,only : &
    m_ESlhxc_potential_3D
use m_Charge_Density,only : &
    chgq_l,m_CD_softpart_3D,m_CD_hardpart,m_CD_cp_chgq_to_chgqo
use m_XC_Potential,only : &
    vxc_l, m_XC_cal_potential_3D
use m_Total_Energy,only : &
    m_TE_total_energy
use m_ES_Intgr_VlhxcQlm,only : &
    m_ESiVQ_integrate_VlhxcQlm_3D
use m_ES_WF_by_MatDiagon,only : &
    m_ESmat_solve_Hx_eq_eSx_3D
use m_rttddft,only : &
    m_rttddft_init_occup_control,m_rttddft_init_impulse_field &
   ,m_rttddft_print_eko_occup &
   ,m_rttddft_propagate_wf_by_taylor &
   ,m_rttddft_dipole,m_rttddft_current
...
call m_ESlhxc_potential_3D(nfout,chgq_l,vxc_l)
call m_ESiVQ_integrate_VlhxcQlm_3D(nfout)

! === DEBUG by tkato 2013/02/24 ================================================
! In RTP-TDDFT, sorting in m_ES_energy_eigen_values_3D brings on difference
! to ORG_Parallel in current.
! The order of band elements on zaj_l is not match to that on occup_l.
! But, this sort is necessary for other case.
!call m_ES_energy_eigen_values_3D(nfout)
call m_ES_energy_eigen_values_3D(nfout, .true.)
! ==============================================================================
