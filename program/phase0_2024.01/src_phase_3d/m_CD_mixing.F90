...
#ifdef _DEBUG_PULAY_
  integer, private,save :: iprintover = 0
#endif
...
!mpi    G_shortest = minval(gr_l(2:kgp))
    ...
    G_shortest_mpi = minval(gr_l(ist:iend_kngp))
                                                 __TIMER_COMM_START_w_BARRIER(mpi_chg_world,1146)
    call mpi_allreduce(G_longest_mpi,G_longest,1 &
                   &  ,mpi_double_precision,mpi_max,mpi_chg_world,ierr)
    call mpi_allreduce(G_shortest_mpi,G_shortest,1 &
                   &  ,mpi_double_precision,mpi_min,mpi_chg_world,ierr)
                                                 __TIMER_COMM_STOP(1146)
...
!!$       gg = gr_l(i)**2
       ...
       if(nrank_chg > 1) then
          call mpi_allreduce(gg,gg_mpi,1,mpi_double_precision,mpi_sum,mpi_chg_world,ierr)
          gg = gg_mpi
       end if
...
!!$             c_p(i,1) = amix_cprec*gg/(gg+q0) * pmix(1)
          ...
          if(myrank_chg == 0) c_p(1,1) = amix_cprec*fg2 * pmix(1)
       end if
    else
  ...
  subroutine precon_4_mult(f_q)
    ...
    if(c_precon ) then
       ist  = ista_kgpm
       if(ist == 1) ist = 2
       if(kgp == kgpm .or. nrank_chg == 1) then
...
!             p = p + f_q(i)*u(i,ik,is)*v(i,ik,is)
       ...
       if( nrank_chg >= 2) then
                                                 __TIMER_COMM_START_w_BARRIER(mpi_chg_world,1161)
          call mpi_allreduce(p,p_mpi,1,mpi_double_precision,mpi_sum,mpi_chg_world,ierr)
                                                 __TIMER_COMM_STOP(1161)
          p = p_mpi
       end if
       fmult(is) = p*univol
    enddo
  ...
  subroutine mult1s5(u,mb,muv,j,iuv,v,f_q,fmult)
    ...
    do is = 1, ndim_magmom, af+1  !    do is = 1, nspin, af+1  ! === modified by K. Tagami === 11.0
       ...
       if( nrank_chg >= 2) then
                                                 __TIMER_COMM_START_w_BARRIER(mpi_chg_world,1163)
          call mpi_allreduce(p,p_mpi,1,mpi_double_precision,mpi_sum,mpi_chg_world,ierr)
                                                 __TIMER_COMM_STOP(1163)
          p = p_mpi
       end if
       fmult(is) = p*univol
    enddo
...
! ========================================================================== 11.0
       ...
       if( nrank_chg >= 2) then
                                                 __TIMER_COMM_START_w_BARRIER(mpi_chg_world,1187)
          call mpi_allreduce(p,p_mpi,1,mpi_double_precision,mpi_sum,mpi_chg_world,ierr)
                                                 __TIMER_COMM_STOP(1187)
          p = p_mpi
       end if
       fmult(is) = p*univol
    enddo
             ...
             do ik = 1, kimg
                ...
                do i = 1, iend_kngp-ista_kngp+1
                   prj_wk(i + ipbase) = c(ista_kngp-1+i,ik,is)  !!$  prj_wk(i,ik,is) = c(ista_kngp-1+i,ik,is)
                                                 ...
                                                 __TIMER_COMM_START_w_BARRIER(mpi_chg_world,1169)
       call mpi_bcast(prj_wk,nelmnt,mpi_double_precision,ip,mpi_chg_world,ierr)
                                                 __TIMER_COMM_STOP(1169)
...
!!$       call mpi_bcast(prj_wk,mp_kngp,mpi_double_precision,ip,MPI_CommGroup,ierr)
       call mpi_bcast(prj_wk,mp_kngp*nspin_m,mpi_double_precision,ip,mpi_chg_world,ierr)
! <--
       ! (2) projection
       istart = ista_kgpm; if(istart < is_kngp(ip)) istart = is_kngp(ip)
       iend   = iend_kgpm; if(iend   > ie_kngp(ip)) iend   = ie_kngp(ip)
...
#ifdef _DEBUG_PULAY_
    if(iprintover == 0) then
       write(6,'("!<<concentrate_d_to_chg>> mdim_magmom = ",i8)') ndim_magmom
       write(6,'("!<<concentrate_d_to_chg>> kgpm, kgp = ",2i8) ') kgpm,kgp
       write(6,'("!<<concentrate_d_to_chg>> nrank_chg = ",i8)') nrank_chg
       iprintover = 1
    end if
#endif       
                   ...
                   do ik = 1, kimg
                      ...
                      do i = istart, iend
                         prj_wk(i+ipbase) = d(i,ik,is)
                                                 ...
                                                 __TIMER_COMM_START_w_BARRIER(mpi_chg_world,1172)
                call mpi_send(prj_wk,nelmnt,mpi_double_precision,ip,1,mpi_chg_world,ierr)
                                                 __TIMER_COMM_STOP(1172)
             end if
...
! === DEBUG by tkato 2011/11/18 ================================================
    if(kgpm < kgp .and. nrank_chg /= 1) deallocate(rho)
    if(kgpm < kgp .and. nrank_chg /= 1) deallocate(rhoo)
    if(kgpm < kgp .and. nrank_chg /= 1) deallocate(c_pm)
! ==============================================================================
...
#ifdef _DEBUG_PULAY_
    write(nfout,'("!**mix_pulay** istrbr = ",i8, " iter = ",i8)') istrbr, iter
    write(nfout,'("!**mix_pulay** sw_recomposing = ",i3)') sw_recomposing
    write(nfout,'("!**mix_pulay** sw_control_stepsize ",i3)') sw_control_stepsize
#endif    
...
! ----
               ...
               if(myrank_chg == 0) u_l_ekinq(1,k,is) = 0.d0
            end do
         end do
      endif
