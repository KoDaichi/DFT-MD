...
!
#ifdef FJ_TIMER
#   define FJ_TIMER_START(a)  call timer_sta(a)
#   define FJ_TIMER_STOP(a)   call timer_end(a)
#else
...
! === m_PP_vanderbilt_type and m_PP_vanderbilt_type_3D look the same!!! by tkato 2011/08/04 ========
       ...
       &                           , m_PP_local_part_3D      &
       &                           , m_PP_partial_core_CD_3D &
       &                           , m_PP_rd_PP_parameters_3D &
  ...
  use m_Const_Parameters,     only : INITIAL,CONTINUATION &
       ...
       &                           , ON, OFF, by_pseudo_atomic_orbitals, ALL_AT_ONCE, pp_CIAOPP, pp_PAW
  use m_Kpoints,              only : kv3, vkxyz, mp_index
  use m_NonLocal_Potential,   only : m_NLP_alloc_snl  &
                                   , m_NLP_betar_dot_PWs_diff_3D,  m_NLP_betar_dot_PWs_3D &
       &                           , m_NLP_rd_snl_3D &
       &                           , m_NLP_alloc_phig, m_NLP_phir_dot_PWs_3D &
       &                           , m_NLP_alloc_snl_add, m_NLP_add_betar_dot_PWs_3D &
       &                           , m_NLP_alloc_paog, m_NLP_paor_dot_PWs_3D &
...
!!$            & ,iatomn(it), univol,nfout,ipri)
                                                  ...
                                                  FJ_TIMER_START(29)
        call m_PP_local_part_3D(it,nfout,gr_l,ngshell,ngshell_range)   ! (psloc), ->(vlocr,psc_l,etot1)
        call m_PP_partial_core_CD_3D(nfout,it,gr_l,paramset=.false.)
                                                  FJ_TIMER_STOP(29)
        !                                      -(m_P.P.) epseud_pc, pcc
...
! =============================================================== 12.1
        ...
        if(istress == OFF) then
           ...
           call m_NLP_betar_dot_PWs_diff_3D(nfout,kv3,vkxyz) !(kbint_diff) --> snl,snld
        else
        ...
        if(intzaj == by_pseudo_atomic_orbitals) then
           call m_NLP_paor_dot_PWs_3D(nfout,kv3,vkxyz) !(kbint) --> paog
        end if
...
!!$     if(istress == ON) call m_PP_alloc_ps_stress()
     call m_PP_rd_PP_parameters_3D(nfout,nfcntn_bin,F_CNTN_BIN_in_partitioned, kgp)
...
!!$ASASASASAS
           call m_PP_partial_core_CD_3D(nfout,it,gr_l,.true.)
!!$ASASASASAS
           call m_Files_close_ps_file(it)
        enddo
...
! ====================================================================== 5.0
  ...
  call m_Parallel_init_mpi_nlmta_3D(nfout,ipriparallel &
...
subroutine PseudoPotential_ek
  ...
  use m_NonLocal_Potential,   only : m_NLP_rd_snl_3D, m_NLP_betar_dot_PWs_3D &
       ...
       &                           , m_NLP_build_snl_in_rspace, m_NLP_alloc_phig &
       &                           , m_NLP_alloc_snl, m_NLP_alloc_snl_add &
       &                           , m_NLP_alloc_paog, m_NLP_paor_dot_PWs_3D
