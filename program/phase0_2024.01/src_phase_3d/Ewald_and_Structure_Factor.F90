!=======================================================================
!
!  PROGRAM  PHASE/0 2020.01 ($Rev: 590 $)
!
...
!$$#ifdef PARA3D
  use m_Ionic_System,     only : m_IS_structure_factor_3D, m_IS_ewald_3D,eewald,fxyzew_l,natm &
     &                         , m_IS_get_extpl_factor, iatomn, ityp, ntyp_vdw, m_IS_vdw, m_IS_vdwdf3
...
! ==============================================================================
   ...
   if(sw_esm==ON)then
      ...
      allocate(agauss(natm,2));agauss=0.d0
      allocate(bgauss(natm,2));bgauss=0.d0
      do i=1,natm
      ...
      call m_IS_ewald_3D(nfout,kg,gr_l,kgp,ngabc,ival,ngabc_kngp_l,alf) ! R only
  ...
  if(ntyp_vdw>0) call m_IS_vdw(nfout)
!$$#endif
