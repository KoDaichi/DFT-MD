...
*     .. External Subroutines ..
     ...
     $                   INFOG2L, PDGEMM, PDLAED2, PDLAEDZ,
     $                   PDLASET, PXERBLA, PDLAED3
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
      real(8) :: t1,t2,z1,z2
...
*
!      call MPI_COMM_RANK ( MPI_COMM_WORLD, MY_RANK, INFO )
!-      print*," MY_PDLAED1 ", MY_RANK
      t1=MPI_Wtime()
...
*     Deflate eigenvalues.
     ...
     $              NN, NN1, NN2, IQ1, IQ2 )
         z2=MPI_Wtime()
!-         print*,"PDLAED2",z2-z1, MY_RANK
*
*
*     Solve Secular Equation.
*

!-         print*,"VALUE K = ", K,  MY_RANK
      IF( K.NE.0 ) THEN
!-         print*,"PDLASET START", MY_RANK
         z1=MPI_Wtime()
         CALL PDLASET( 'A', N, N, ZERO, ONE, WORK( IPU ), 1, 1, DESCU )
         z2=MPI_Wtime()
!-         print*,"PDLASET",z2-z1, MY_RANK
...
!         PRINT *, "IN 1", K, N, NB
     ...
     $                 IWORK( INDXC ), IWORK( ICTOT ), NPCOL, INFO )
         z2=MPI_Wtime()
!-         print*,"PDLAED3",z2-z1, MY_RANK

!      print*,WORK(IDLMDA:IDLMDA+N-1)
!        PRINT*,"IMATRIX=",IQ-1+ID,JQ-1+ID
!      print*,WORK(IPU:IPU+36)

      IF (MYROW.EQ.0 .AND. MYCOL.EQ.0) THEN    
!         PRINT *, "IN 1", D(1), D(2), D(3), D(4)
      END IF   
*
...
!-         print*,N1,NN,NN1
     ...
     $                   ZERO, Q, INQ, JNQ, DESCQ )
         z2=MPI_Wtime()
!-         print*,"  PDGEMM-1(1)",z2-z1, MY_RANK
         END IF
...
!-         print*,N-N1,NN,NN2
     ...
     $                   DESCU, ZERO, Q, INQ, JNQ, DESCQ )
         z2=MPI_Wtime()
!-         print*,"  PDGEMM-1(2)",z2-z1, MY_RANK
         END IF
