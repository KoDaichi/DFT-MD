...
*
         ...
         IF( NN1.GT.0 ) THEN
            ...
            JNQ = JQ - 1 + ID + IQQ - 1
            d1 = MPI_WTIME()
! Q := WORK(IPQ2)(1,IQ1)[N1,NN1] * WORK(IPU)(IQ1,IQQ)[NN1,NN]
            CALL PDGEMM( 'N', 'N', N1, NN, NN1, ONE, WORK( IPQ2 ), 1,
     $                   IQ1, DESCQ2, WORK( IPU ), IQ1, IQQ, DESCU,
     $                   ZERO, Q, INQ, JNQ, DESCQ )
            d2 = MPI_WTIME()
         flops = flops+(2*DBLE(N1)*NN*NN1)
         dgemm_time = dgemm_time+(d2-d1)
!=!         if(MYRANK==0) then
!=         print*,MYRANK,
!=     &   "(",N1,NN1,")@[",1,IQ1,"]x(",NN1,NN,")@[",IQ1,IQQ,"]"
!=!         endif
         END IF
         IF( NN2.GT.0 ) THEN
            INQ = IQ - 1 + ID + N1
...
!=!         endif
         END IF
*
!$OMP PARALLEL DO PRIVATE(J,JC,JJC,JJ2C,COL)
         DO 10 J = K + 1, N
            JC = IWORK( INDX+J-1 )
