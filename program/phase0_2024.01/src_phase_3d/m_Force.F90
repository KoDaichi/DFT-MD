...
! $Id: m_Force.F90 606 2020-04-15 06:45:49Z ktagami $
  ...
  use m_Const_Parameters,     only : DP, zi, EXECUT, SKIP, PAI, PAI2, PAI4, DELTA, ON&
       ...
       &                           , BONDLENGTH_FIX_2, COG_FIX, COG_FIX_L, RIGID_BODY_FIX_IN_A_PLANE, HEAT_BATH, VXC_AND_EXC &
...
! -->
  ...
  use m_Parallelization,      only : ista_g1k, iend_g1k, mpi_ke_world &
       &                           , ista_k, iend_k, np_g1k, mpi_kg_world  &
       &                           , mpi_chg_world
  use m_PlaneWaveBasisSet,   only : ngabc_kngp_l, ngabc_kngp_B_l
!fj$$F  use m_Electronic_Structure, only : zaj_l_3D, nrvf_ordr, fsr_l_3D, fsi_l_3D, eko_l_3D,  occup_l_3D
  use m_Electronic_Structure, only : nrvf_ordr
!f  use z_interface_3D,         only : decomp_zaj_l_3D_ik, decomp_fsr_l_3D_ik &
!f                                   , decomp_eko_l_3D_new
! === fs[ri]_3D -> fs[ri]_2D =============================================================
  use m_Electronic_Structure, only : fsr_l_2D, fsi_l_2D &
       &                           , m_ES_alloc_fsri_l_2D, m_ES_dealloc_fsri_l_2D
! ========================================================================================
...
! === DEBUG by T.Kato 2013/07/17 ===============================================
  use m_Parallelization,        only : np_fs, mpi_ge_world
! ==============================================================================
...
!!$               & ,iwei, forc_l, forcw, ipfrc, iop_supercell)
    ...
    call mpi_bcast(forc_l,3*natm,mpi_double_precision,0,MPI_CommGroup,ierr)
#ifdef __TIMER_SUB__
    call timer_end(1506)
#endif
       ...
       do ia = 1, natm
          ...
          if(imdtyp(ia) == RELAX .or. imdtyp(ia) > HEAT_BATH) then
             ...
             if(forcmx < fca) forcmx = fca
               ...
               & .or. imdtyp(ia) == BONDLENGTH_FIX_2 .or. imdtyp(ia) == COG_FIX .or. imdtyp(ia) == COG_FIX_L) then
             fca = dsqrt(forc_l(ia,1)**2+forc_l(ia,2)**2+forc_l(ia,3)**2)
             if(forcmx < fca) forcmx = fca
          end if
       end do
!xocl end spread sum(forcmx)
  ...
  subroutine extract_largerforce_atoms(nforc, ndim, iatom_larger)
    ...
    do ia = 1, natm
       if(imdtypxyz(ia,1)==0 .and. imdtypxyz(ia,2)==0 .and. imdtypxyz(ia,3)==0) cycle
       it = imdtyp(ia)
       if(it == RIGID_BODY_FIX_IN_A_PLANE) cycle
       if(it == RELAX .or. it > HEAT_BATH .or. it == BONDLENGTH_FIX .or. it == BONDLENGTH_FIX_1 .or. &
            & it == BONDLENGTH_FIX_2 .or. it == COG_FIX .or. it == COG_FIX_L) then
...
#ifdef ENABLE_ESM_PACK
    if(sw_esm==ON)then
       allocate(aux(nfftps/kimg));aux(:) = (0.d0,0.d0)
       ...
       call mpi_allreduce(MPI_IN_PLACE,aux,nfftps/kimg,mpi_double_complex,mpi_sum,mpi_chg_world,ierr)
...
!CDIR INNER
            ...
            do i = ista_kngp, iend_kngp  !for mpi
               ...
               f1_mpi(1) = f1_mpi(1) + tp*ngabc_kngp_l(i,1)
               f1_mpi(2) = f1_mpi(2) + tp*ngabc_kngp_l(i,2)
               f1_mpi(3) = f1_mpi(3) + tp*ngabc_kngp_l(i,3)
            end do
         else if(kimg == 2) then
!!$#ifdef NEC_TUNE_SMP
!!$!CDIR INNER
...
!f           &  ,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
  ...
  call timer_sta(1534)
      ...
      if(kimg == 1) then
         do i = ista_kngp, iend_kngp  !for mpi
            ...
            f2_mpi(1) = f2_mpi(1) + tp*ngabc_kngp_l(i,1)
            f2_mpi(2) = f2_mpi(2) + tp*ngabc_kngp_l(i,2)
            f2_mpi(3) = f2_mpi(3) + tp*ngabc_kngp_l(i,3)
         end do
      else if(kimg == 2) then
         do i = ista_kngp, iend_kngp  !for mpi
...
!CDIR INNER
            ...
            do i = ista_kngp, iend_kngp  !for mpi
               ...
               fx = fx + tmp*ngabc_kngp_l(i,1)
               fy = fy + tmp*ngabc_kngp_l(i,2)
               fz = fz + tmp*ngabc_kngp_l(i,3)
            end do
         else if(kimg == 2) then
#ifdef NEC_TUNE_SMP
!CDIR INNER
...
!f       call mpi_allreduce(flhxcq_l,flhxcq_mpi,3*natm,mpi_double_precision &
...
!!$               f  = qitg_l(i,iq)*ylm(iy)*vlhxc_l(i,1,is)*zfsc1(iy)
               ...
               tx = tx + f*ngabc_kngp_l(i,1);  ty = ty + f*ngabc_kngp_l(i,2)
               tz = tz + f*ngabc_kngp_l(i,3)
            end do
         else if(kimg == 2) then
#ifdef NEC_TUNE_SMP
!CDIR INNER
...
!f         call mpi_allreduce(f1_mpi,f2_mpi,3 &
!f              &  ,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr) ! MPI
#ifdef __TIMER_COMM__
  call timer_barrier(mpi_chg_world)
  call timer_sta(1512)
#endif
         call mpi_allreduce(f1_mpi,f2_mpi,3 &
              &  ,mpi_double_precision,mpi_sum,mpi_chg_world,ierr) ! MPI
#ifdef __TIMER_COMM__
  call timer_end(1512)
#endif
...
!f    real(kind=DP),allocatable,dimension(:,:)     :: occup_l_3D
!f#endif
...
!f#ifdef PARA3D
      ...
      allocate( work1(np_e,nlmta,3) )    ! MPI
!f      allocate(occup_l_3D(np_e,ista_k:iend_k)) ; occup_l_3D = 0.d0
!f#else
...
!f#ifdef PARA3D
! ==============================================================================
!fj$    do is = 1, nspin, af+1
!fj$       do ik = is, kv3-nspin+is, nspin
!fj$          if(map_k(ik) /= myrank_k) cycle
!fj$          call decomp_zaj_l_3D_ik(zaj_l,zaj_l_3D,ik,nrvf_ordr,"    ")
!fj$          call decomp_fsr_l_3D_ik(fsr_l,fsr_l_3D,ik,nrvf_ordr,"    ")
!fj$          if(.not.(kv3/nspin == 1 .and. k_symmetry(1) == GAMMA .and. kimg == 2)) then
!fj$             call decomp_fsr_l_3D_ik(fsi_l,fsi_l_3D,ik,nrvf_ordr,"    ")
!fj$          endif
!fj$          call decomp_eko_l_3D_new(eko_l,eko_l_3D,ik,nrvf_ordr,"    ")
!fj$          call decomp_eko_l_3D_new(occup_l,occup_l_3D,ik,nrvf_ordr,"    ")
!fj$       enddo
!fj$    enddo
! ==============================================================================
      do is = 1, nspin, af+1
...
!!$            endif
            call m_ES_alloc_fsri_l_2D(ik)

            call gather_f_3d_to_2d(fsr_l, fsr_l_2D)
            if( k_symmetry(ik) /= GAMMA ) then
               call gather_f_3d_to_2d(fsi_l, fsi_l_2D)
            endif
! ========================================================================================
  ...
  call timer_end(1544)
            ...
            call m_ES_dealloc_fsri_l_2D(ik) ! deallocate(fsr_l_2D, fsi_l_2D)
         end do
! === DEBUG by tkato 2014/10/23 ================================================
!        call mpi_allreduce(MPI_IN_PLACE,fnlxyz_l,3*natm,mpi_double_precision,mpi_sum,mpi_ke_world,ierr)
! ==============================================================================

      end do
! === DEBUG by T.Kato 2014/10/23 ===============================================
!     call mpi_allreduce(MPI_IN_PLACE,fnlxyz_l,3*natm,mpi_double_precision,mpi_sum,mpi_ge_world,ierr)
! ==============================================================================
...
!            & , mpi_sum, mpi_kg_world,ierr)
         call mpi_allreduce(fnlxyz_l,fnlxyz_mpi,3*natm,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
! ==============================================================================
#ifdef __TIMER_COMM__
  call timer_end(1515)
#endif
...
!     real(kind=DP), dimension(:,:)  , intent(out) :: out
      real(kind=DP), dimension(np_e,np_fs,ista_k:iend_k), intent(in)  :: inn
      real(kind=DP), dimension(np_e,nlmta),               intent(out) :: out
! ==============================================================================
    ...
    call timer_sta(1517)
      ...
      if(ierr /= 0) then
         ...
         call phase_error_with_msg(nfout,'Error Not allocated',__LINE__,__FILE__)
      endif
...
!                   fi1 = fsi_l_3D(i,lmta1,ik); fi2 = fsi_l_3D(i,lmta2,ik)
  ...
  call timer_end(1549)
    ...
    call timer_end(1521)
      ...
      subroutine pre_drv_of_betar_dot_WFs_div()
          integer :: ia, i, t, lmt1, lmtt1, icnt, mil, fac_tmp, isft
    ...
    call timer_sta(1522)
          ...
          icnt = 0
          isft = ista_g1k(ik)-1
          if(ibl1-isft<1 .or. ibl2-isft>maxval(np_g1k)) then
             write(nfout,'(" range of (ibl1:ibl2) is illegal in <pre_drv_of_betar_dot_WFs_div> in <<.._3D>>")')
             write(nfout,'("  ibl1-ista_g1k(ik)+1, ibl2-ista_g1k(ik)+1, maxval(np_g1k) = ",3i8)') &
                  & ibl1-ista_g1k(ik)+1, ibl2-ista_g1k(ik)+1, maxval(np_g1k)
             call phase_error_with_msg(nfout,' range of (ibl1:ibl2) is illegal in <pre_drv_of_betar_dot_WFs_div> in <<.._3D>>'&
                                      ,__LINE__,__FILE__)
          end if
             ...
             if(kimg == 1) then   ! only ar
               do ia = 1, natm
                  ...
                  do lmt1 = 1, ilmt(it)
                     ...
                     lmtt1 = lmtt(lmt1,it)
                     do i = ibl1, ibl2
                        ar_tmp(i-ibl1+1,icnt,1) = zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,1)
                        ar_tmp(i-ibl1+1,icnt,2) = zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,2)
                        ar_tmp(i-ibl1+1,icnt,3) = zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,3)
                     end do
                  enddo
               enddo
             else if(kimg == 2) then
               do ia = 1, natm
                  ...
                  do lmt1 = 1, ilmt(it)
                     ...
                     if( mil.eq.1 .or. mil.eq.3 ) then
                         do i = ibl1, ibl2
                            ar_tmp(i-ibl1+1,icnt,1) =  fac_tmp*zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,1)
                            ar_tmp(i-ibl1+1,icnt,2) =  fac_tmp*zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,2)
                            ar_tmp(i-ibl1+1,icnt,3) =  fac_tmp*zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,3)
                            ai_tmp(i-ibl1+1,icnt,1) =  fac_tmp*zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,1)
                            ai_tmp(i-ibl1+1,icnt,2) =  fac_tmp*zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,2)
                            ai_tmp(i-ibl1+1,icnt,3) =  fac_tmp*zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,3)
                         end do
                     else if( mil.eq.0 .or. mil.eq.2 ) then
                         do i = ibl1, ibl2
                            ar_tmp(i-ibl1+1,icnt,1) = -fac_tmp*zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,1)
                            ar_tmp(i-ibl1+1,icnt,2) = -fac_tmp*zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,2)
                            ar_tmp(i-ibl1+1,icnt,3) = -fac_tmp*zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,3)
                            ai_tmp(i-ibl1+1,icnt,1) =  fac_tmp*zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,1)
                            ai_tmp(i-ibl1+1,icnt,2) =  fac_tmp*zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,2)
                            ai_tmp(i-ibl1+1,icnt,3) =  fac_tmp*zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,3)
                         end do
                     endif
                  enddo
                enddo
                  ...
                  do lmt1 = 1, ilmt(it)
                     ...
                     mil_tmp(icnt) = mod( ltp(lmt1,it),4 )
                      do i = ibl1, ibl2
                         ...
                         ar_tmp(i-ibl1+1,icnt,2) = zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,2)
                         ar_tmp(i-ibl1+1,icnt,3) = zfcos(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,3)
                         ai_tmp(i-ibl1+1,icnt,1) = zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,1)
                         ai_tmp(i-ibl1+1,icnt,2) = zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,2)
                         ai_tmp(i-ibl1+1,icnt,3) = zfsin(i-ibl1+1,ia)*snl(i-isft,lmtt1,iksnl)*ngabc_red(i-ibl1+1,3)
                      end do
                  enddo
                enddo
#ifdef __TIMER_DO__
  ...
  call timer_sta(1552)
         ...
         if(k_symmetry(ik) == GAMMA) then
            if(kimg == 1) then
               nblk=ibl2-ibl1+1
               nblk2=ibl1-ista_g1k(ik)+1
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ar_tmp(1,1,1),ibsize, &
              &           0.d0,gr(1,1,1),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ar_tmp(1,1,2),ibsize, &
              &           0.d0,gr(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ar_tmp(1,1,3),ibsize, &
              &           0.d0,gr(1,1,3),np_e)
            else if(kimg == 2) then
               ibl1_2 = max(2,ibl1)
               nblk =ibl2-ibl1_2+1
               nblk2=ibl1_2-ista_g1k(ik)+1
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(ibl1_2-ibl1+1,1,1), &
              &           ibsize,0.d0,gr(1,1,1),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ar_tmp(ibl1_2-ibl1+1,1,1), &
              &           ibsize,1.d0,gr(1,1,1),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(ibl1_2-ibl1+1,1,2), &
              &           ibsize,0.d0,gr(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ar_tmp(ibl1_2-ibl1+1,1,2), &
              &           ibsize,1.d0,gr(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(ibl1_2-ibl1+1,1,3), &
              &           ibsize,0.d0,gr(1,1,3),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ar_tmp(ibl1_2-ibl1+1,1,3), &
              &           ibsize,1.d0,gr(1,1,3),np_e)
            endif
         else
            if(kimg == 1) then
               nblk =ibl2-ibl1+1
              ...
              &           0.d0,gr(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ar_tmp(1,1,3),ibsize, &
              &           0.d0,gr(1,1,3),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(1,1,1),ibsize, &
              &           0.d0,gi(1,1,1),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(1,1,2),ibsize, &
              &           0.d0,gi(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(1,1,3),ibsize, &
              &           0.d0,gi(1,1,3),np_e)
            else if(kimg == 2) then
               nblk =ibl2-ibl1+1
               nblk2=ibl1-ista_g1k(ik)+1
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ar_tmp(1,1,1),ibsize, &
              &           0.d0,gr(1,1,1),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk,-1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ai_tmp(1,1,1),ibsize, &
              &           1.d0,gr(1,1,1),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ar_tmp(1,1,2),ibsize, &
              &           0.d0,gr(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk,-1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ai_tmp(1,1,2),ibsize, &
              &           1.d0,gr(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ar_tmp(1,1,3),ibsize, &
              &           0.d0,gr(1,1,3),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk,-1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ai_tmp(1,1,3),ibsize, &
              &           1.d0,gr(1,1,3),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(1,1,1),ibsize, &
              &           0.d0,gi(1,1,1),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ar_tmp(1,1,1),ibsize, &
              &           1.d0,gi(1,1,1),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(1,1,2),ibsize, &
              &           0.d0,gi(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ar_tmp(1,1,2),ibsize, &
              &           1.d0,gi(1,1,2),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,1),maxval(np_g1k),ai_tmp(1,1,3),ibsize, &
              &           0.d0,gi(1,1,3),np_e)
               call DGEMM('T','N',np_e,nlmta,nblk, 1.d0,zaj_l(nblk2,1,ik,2),maxval(np_g1k),ar_tmp(1,1,3),ibsize, &
              &           1.d0,gi(1,1,3),np_e)
            end if
#ifdef __TIMER_DO__
  call timer_sta(1553)
