...
! --> T. Yamasaki 17th Aug. 2009
  subroutine m_CS_fix_total_spin(nfout)
    integer,       intent(in) :: nfout 
...
1001  continue
      if(.not.(imag == NONMAG .or. imag == ANTIFERRO .or. imag == FERRO .or. imag == NONCOLLINEAR)) then
         if(printable) then
            write(nfout,'("!ERROR A setting in the inputfile as (( magnetic_state = ",a10," )) is not proper.")') rstr
            write(nfout,'("!ERROR  The value must be nonmag (=para), magnetic (=ferro), antiferro or noncollinear.")')
         end if
    ...
    subroutine set_imag_from_nspin(iret)
      ...
      if(.not.(iret==1 .or. iret==2 .or. iret==-2 .or. iret==4)) then
         if(printable) then
            write(nfout,'("!ERROR A setting in the inputfile as (( nspin = ", i4, " )) is not proper.")') iret
            write(nfout,'("!ERROR  The value must be 1, 2,-2, or 4")')
         end if
         call phase_error_with_msg(nfout,'(!** nspin is not given properly in the inputfile' &
              & ,__LINE__,__FILE__)
      end if
