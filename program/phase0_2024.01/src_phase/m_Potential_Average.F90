module m_Potential_Average
  ...
  use m_PlaneWaveBasisSet,  only : ngabc, gr_l, kgp
...
!          rad1 = rad_for_pot_avg(it)
          ...
          Do is=1, ismax
             csum = 0.0d0
             Do i=ist, iend_kngp
                VecG(1) = rltv(1,1)*ngabc(i,1) +rltv(1,2)*ngabc(i,2) &
                     &   +rltv(1,3)*ngabc(i,3)
                VecG(2) = rltv(2,1)*ngabc(i,1) +rltv(2,2)*ngabc(i,2) &
                     &   +rltv(2,3)*ngabc(i,3)
                VecG(3) = rltv(3,1)*ngabc(i,1) +rltv(3,2)*ngabc(i,2) &
                     &   +rltv(3,3)*ngabc(i,3)
