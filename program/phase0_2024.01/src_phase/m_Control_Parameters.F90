...
!!$       &, FFT_redundant, FFT_parallel &
       ...
       &, BOHR
! ==============================================================================
  use m_Parallelization, only   : MPI_CommGroup,npes,mype,ierr,nrank_e,nrank_k, nrank_s
...
!
  ...
  character(len("tag_previous_nopr")), private,parameter ::  &
       &        tag_previous_nopr = "previous_nopr"
  integer :: previous_nopr_collinear = 0
! ===================================================================== 11.0
...
!!$  integer       :: sw_recomposing = NO
  ...
  integer, private :: print_sw_recomposing = 0
! <-
...
!!$  integer :: sw_force_simple_mixing=ON    ! 2024.06.04 by T. Yamasaki
...
! -- PROCAR --
  ...
  character(len("procar_sph_zaxis_to_magdir")),private,parameter :: &
       &         tag_procar_sph_zaxis_to_magdir = "procar_sph_zaxis_to_magdir"
  character(len("procar_norm_zaxis_to_magdir")),private,parameter :: &
       &         tag_procar_norm_zaxis_to_magdir = "procar_norm_zaxis_to_magdir"
  integer :: procar_sph_zaxis_to_magdir = OFF
  integer :: procar_norm_zaxis_to_magdir = OFF
       ...
       if( f_selectBlock( tag_hybrid_functional) == 0) then
          ...
          if(sw_hybrid_functional == ON) then
             if( PAW_switch == ON )then
               call phase_error_with_msg(nfout,'!** PAW method cannot be used in conjunction with '//&
                                               'the hybrid functional.',__LINE__,__FILE__)
               !if(printable) write(nfout,'(a)') ' !** PAW method cannot be used in &
               !                       & conjunction with the hybrid functional.'
               !stop                             ' !** PAW method cannot be used in &
               !                       & conjunction with the hybrid functional.'
             endif
             ...
             if(f_getStringValue(tag_qr_optimization,rstr,LOWER)==0)then
                ...
                elseif(rstr==tag_none)then
                   ...
                   !stop
                   call phase_error_with_msg(nfout,'unsupported projector optimization method : '//trim(rstr), &
                   __LINE__,__FILE__)
                endif
             endif
	     ...
	     tf = ( f_getIntValue(tag_sw_submat,iret) == 0)
             ...
             if(tf) then
                if(trim(rstr) == trim(tag_lmMSD)) then
                   ...
                   call phase_error_with_msg(nfout, ' ! solver for positronWF is not given properly'// &
                                                    '<<m_CtrlP_rd_accuracy>>',__LINE__,__FILE__)
                end if
             end if
...
! ====================== vdW-DF ==========================
       ...
       !stop ' tag_accuracy is not given in the inputfile <<CtrlP_rd_accuracy>>'
       call phase_error_with_msg(nfout,' tag_accuracy is not given in the inputfile <<CtrlP_rd_accuracy>>', &
       __LINE__,__FILE__)
    end if
#ifndef _EMPIRICAL_
  contains
...
!!$         stop ' !! no tag line for cutoff_energy_for_chargedensity'
  ...
  ! ------- Positron start
    subroutine getgmax_positron()
      ...
      if(tf) then
         if(dret < 0) call phase_error_with_msg(nfout,' !! illegal input value of cutoff-energy for a positron wave'//&
  ...
  ! ------- Positron end
    ...
    subroutine getgmaxs()
      ...
      if(.not.tf) tf = f_getRealValue( tag_cke_wf3, dret, "rydberg") == 0
      if(tf) then
         if(dret < 0) call phase_error_with_msg(nfout,' !! illegal input value of '//&
                                                      'cutoff_energy_for_initial_matdiagon',__LINE__,__FILE__)
...
1001  continue
    ...
    subroutine set_intzaj(rstr)
      ...
      if(tf) then
         intzaj = by_matrix_diagon; goto 1001
      end if
...
!!$        explict_solver = f_selectBlock(tag_solvers)==0
    ...
    if(flag) then
       ...
       if( f_selectBlock( tag_for_init_str) == 0) then
          ...
          call set_wfsolvers(prealloc,n_WF_solvers_before,0,ba,iret)
          if(iret < 0) call phase_error_with_msg(nfout,&
          ' solver set is not given properly <<m_CtrlP_rd_wfsolver>>',__LINE__,__FILE__)
          if(.not.number_is_given) n_WF_solvers_before = iret
          if(number_is_given .and. n_WF_solvers_before > iret) n_WF_solvers_before = iret
          n_WF_solvers_after = 0
          if(ipriinputfile >= 2 .and. printable) &
...
!!$          if(ipriinputfile >= 2 .and. printable) &
  ...
  contains
    subroutine set_energy_evaluation(rstr)
      ...
      ' ! tag for energy_evaluation is invalid <<m_CtrlP_rd_wfsolver.set_energy_evaluation>>',__LINE__,__FILE__)
1001  continue
...
!!$                  end if
         ...
         call phase_error_with_msg(nfout, &
        ' ! No wf solver is given in the inputfile <<m_CtrlP_rd_wfsolver.set_wfsolvers>>',__LINE__,__FILE__)
      end if
        iret = i-1
       ...
       if(f_selectBlock(tag_nnp_output) == 0) then
          ...
          if(sw_nnp_output == ON) then
             ...
             if(printable .and. ipriinputfile>=1) then
                ...
                write(nfout,'(a)')        ' !** nnp filetype  = XSF, N2P2 and DEEPMD'
                endif
...
! --- procar ---
       if( f_selectBlock( tag_procar ) == 0) then
          ...
          if ( sw_write_procar_file == ON ) then
             ...
             if ( f_getIntValue( tag_procar_sph_zaxis_to_magdir, iret ) == 0 ) then
                procar_sph_zaxis_to_magdir = iret
                write(nfout,*) '!** procar_sph_zaxis_to_magdir is set to ', &
                     &          procar_sph_zaxis_to_magdir
             endif
             if ( f_getIntValue( tag_procar_norm_zaxis_to_magdir, iret ) == 0 ) then
                procar_norm_zaxis_to_magdir = iret
                write(nfout,*) '!** procar_norm_zaxis_to_magdir is set to ', &
                     &          procar_norm_zaxis_to_magdir
             endif
          endif
          iret = f_selectParentBlock()
       end if
...
1001  continue
  ...
  subroutine m_CntrlP_set_crtdst(a,b,c)
    ...
    if (sw_calc_wf_atom_decomposition==ON) then
       crtdst_aldos = min(a,b,c)/2.0
       write(6,'(" crtdst = ",f8.4)') crtdst_aldos
    end if
    if (sw_calc_wf_layer_decomposition==ON) then
       if(normal_axis_winlay == 1) then
          crtdst_winlay = a/2;
...
!!$          stop ' tag_edelta_ontheway is not found'
    ...
    if(printable) write(nfout,'(i10, " : neg_previous")') neg_previous
...
1      continue
  ...
  subroutine m_CtrlP_wd_cpu_total()
    real(kind=DP)       :: wct_now, cpu_total
#ifdef _CHECK_ELAPSE_
    integer :: iremain
    call chkelaps(iremain)
...
#endif
    if(npes > 1) call mpi_bcast(cpu_total,1,mpi_double_precision,0,MPI_CommGroup,ierr)
    if(printable) write(6,'(" <<Total CPU TIME (elapsed time) = ",f15.5, " (sec.)>>")') cpu_total
...
!  end subroutine m_CtrlP_set_nspin_and_af
  ...
  subroutine m_CtrlP_set_nspin_and_af( nfout,imag )
    ...
    case (NONCOLLINEAR)
       ...
       hardpart_subroutine = 0
    end select
!
    if(noncol .and. nrank_s>1) then
      call phase_error_with_msg(nfout, 'spin parallelization cannot be used in conjunction with magnetic_state = noncollinear' &
      &                         , __LINE__, __FILE__)
    endif
    if ( noncol ) then
...
!!$    if(neg*2 < totch + 4.0) neg = ceiling(totch/2) + 2
    ...
    if(ekmode == ON .or. ekmode == GRID .or. &
    & icond==FIXED_CHARGE .or. icond==FIXED_CHARGE_CONTINUATION .or. icond==FIXED_CHARGE_AUTOMATIC .and. &
    & .not. num_extra_bands_was_set) then
      call m_CtrlP_set_def_numextrabands(neg)
    endif
...
!    if ( libxc_exch_id == 208 ) vtau_exists = .false.
