...
!
  use m_PlaneWaveBasisSet,    only : ngabc,gr_l,igfp_l,kg,kgp,kgp_reduced, ylm_l&
       &                           , m_pwBS_sphrp2
  use m_PlaneWaveBasisSet,    only : m_pwBS_sphrp2_diff
#ifdef _MPIFFT_
...
!    if ( noncol ) deallocate( Rot_Angles )
...
!            call cpafft(iloop)                     ! chgrhr_l -> afft
            call cp_vals_to_afft_rspace( iloop, chgrhr_l, afft )    ! chgrhr_l -> afft
         ...
         if ( use_metagga .and. vtau_exists ) then
            if (myrank_ggacmp < nrank_ggacmp) then
               call cp_vals_to_afft_rspace( iloop, dF_dtau, bfft )
...
! ========================================================================== 11.0
    ...
    subroutine calc_lapl_dF_dlapl
      ...
      Do iloop=1, nspin
         if(myrank_ggacmp < nrank_ggacmp) then
            call cp_vals_to_afft_rspace( iloop, dF_dlaplrho, afft )
            call m_FFT_CD_direct_c(nfout,afft)     ! afft(R space) -> afft(G sp.)
         else
            call m_FFT_CD_direct_c(nfout,afft,mode=0)
         end if
...
!!$         deallocate(np_send,np, ip)
    ...
    subroutine cp_vals_to_afft_rspace( is, vals, afft )
      integer, intent(in) :: is
    ...
    end subroutine cp_vals_to_afft_rspace
...
! -----  ascat stars modifying  -----
    subroutine cp_exc_for_EDA
      ...
      exc_on_a_grid = 0.d0
      do i = ista_fftph, iend_fftph                  ! MPI
        !exc_on_a_grid(2*i-1) = univol*exc_on_a_grid_wk(i)
         exc_on_a_grid(2*i-1) = univol*exc_on_a_grid_wk(i)/f2or1(i)
      end do
