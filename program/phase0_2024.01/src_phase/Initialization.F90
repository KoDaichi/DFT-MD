...
! $Id: Initialization.F90 622 2020-05-26 05:22:07Z jkoga $
  ...
  use m_Parallelization, only : nrank_e, nrank_k, nrank_g1, read_from_args
...
!  call m_Parallel_get_nproc_from_arg(printable)
  if ((ne.ge.1) .and. (nk.ge.1) .and. (npes .eq. (ne*nk))) then
    nrank_e  = ne
    nrank_k  = nk
    nrank_g1 = ne
    read_from_args = .true.
    write(nfout,'(a,3i8)') '!** np ne nk ',npes,nrank_e,nrank_k
  else
    nrank_e  = 1
    nrank_k  = 1
    nrank_g1 = 1
...
!    write(vers,'("phase/0 2021.01 Revision:",i5," -- ORG_Parallel --")') commit_id
    vers = "phase/0 2024.01 Revision:"//commit_id//" -- ORG_Parallel --"
    codename = 'phaseUnif'
    system = ''
...
! $Id: Initialization.F90 622 2020-05-26 05:22:07Z jkoga $
  use m_Parallelization, only : m_Parallel_init_comm_world, npes
  use m_Parallelization, only : m_Parallel_get_nproc_from_arg, nrank_e, nrank_k, nrank_g1, read_from_args
