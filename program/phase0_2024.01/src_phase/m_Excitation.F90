...
!
  use m_Const_Parameters,   only :  ON, OFF, DP, CMPLDP, LOWER, FMAXVALLEN, zi, &
       &                            ELECTRON, DIRECT, BUCS, PAI2, PAI4, Hartree, YES
...
! --------------------------------------------------------
  ...
  subroutine m_XI_wd_WAVEDERF
    ...
    use m_Kpoints, only : m_Kp_set_star_of_k, kv3_ek
      ...
      if ( mype == 0 ) then
         ...
         if ( ekmode == ON ) then
            if ( nk_in_the_process == 1 ) then
               open( lun, file=file0, status="unknown", form="formatted" )
               write(lun,*) nspin, kv3_ek/nspin, neg
            else
               open( lun, file=file0, status="unknown", form="formatted", &
                    &     position = 'append' )
            endif
            ...
            open( lun, file=file0, status="unknown", form="formatted" )
            if ( sw_wavederf_full_bz == ON ) then
               write(lun,*) nspin, kv3_fbz/nspin, neg
            else
               write(lun,*) nspin, kv3/nspin, neg
            endif
         endif
#endif
      endif
