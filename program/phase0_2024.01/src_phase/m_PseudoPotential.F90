...
!  62. m_PP_set_index_arrays2              <-(m_Charge_Density, m_ES_Intgr_VlhxcQlm, m_Force)
...
320 FORMAT(' !PP TOTAL PARTIAL CHARGE         SCHGPC = ',F20.10)
  ...
  subroutine m_PP_wd_PP_parameters(nfout,nfcntn_bin,F_CNTN_BIN_partitioned,kgp)
    ...
    subroutine wd_kgp_array(n1,n2,reduced_array)
      ...
      if(n2 >= 1) then
         if(n1*n2*8 > MAXIMUM_MPI_SIZE) then
            if(ipripp >= 3) write(nfout,'(" n1, n2 = ",i10,",",i8, &
...
!!$         write(nfcntn_bin) reduced_array(ista:iend,n2)
  ...
  end subroutine m_PP_wd_PP_parameters
  ...
  subroutine m_PP_rd_PP_parameters(nfout,nfcntn_bin,F_CNTN_BIN_partitioned, kgp)
    integer, intent(in) :: nfout,nfcntn_bin,kgp
    logical, intent(in) :: F_CNTN_BIN_partitioned
    integer             :: ista, iend, it, lmt1, lmt2
    integer             :: id_sname = -1
    ...
    if(F_CNTN_BIN_partitioned) then
       ...
       if(ipripp >= 3) write(nfout,'(" !PP corecharge_cntnbin = ",i8)') corecharge_cntnbin
       if(corecharge_cntnbin == ON) then
          if(sw_positron /= OFF) then
  ...
  contains
    subroutine rd_kgp_array(n1,n2,reduced_array)
      ...
      if(n2 >= 1) then
         if(n1*n2*8 > MAXIMUM_BCAST_SIZE) then
            ...
            do j = 1, n2
               do i = ista, iend                   ! MPI
                  reduced_array(i,j) = a_mpi(i,j)  ! MPI
               end do
            end do
            deallocate(a_mpi)
         end if
    ...
    subroutine rd_kgp_array_p(n2,reduced_array)
      ...
      if(n2 >= 1) then
         ...
         do j = 1, n2
            do i = ista, iend                          ! MPI
               reduced_array(i,j) = a_wk(i-ista+1,j)  ! MPI
            end do
         end do
         deallocate(a_wk)
      end if
  ...
  end subroutine m_PP_rd_PP_parameters
...
! ==============================================================================
  ...
  subroutine m_PP_dealloc_ps_ntyp
    ...
    if (intzaj == by_pseudo_atomic_orbitals ) then
       deallocate(lmtt_pao);       deallocate(ltp_pao)
       deallocate(mtp_pao);       deallocate(taup_pao);       deallocate(irank)
    end if
...
! =============== 2014/09/19
  ...
  subroutine m_PP_partial_core_CD(nfout,it,gr_l,paramset)
...
! ============== KT_add === 13.0S
#ifndef PCC_with_PAW_OLD
    if ( sw_remove_pcc_from_pawpot == ON ) then
       if ( flg_paw .and. ipaw(it) == 1 .and. itpcc(it) == 1 ) then
          rhpcrpw(:,it) = 0.0d0
       endif
    endif
#endif
! ========================= 13.0S
    ...
    if(itpcc(it) == 1) then
       if(ipripp >= 3) write(nfout,'(" !PP itpcc(",i5," ) = ",i5)') it,itpcc(it)
       mmp = mmp + 1
       itpcc(it) = mmp
       ntpcc     = mmp
       if(.not.paramset) then
          ...
          elseif(istress == 1) then
             ...
             if(ipripp >= 2) write(nfout,'(" !PP pcc - istress = ",i5)') istress
...
#ifndef PCC_with_PAW_OLD
         if ( ipaw(it) == 0 ) then
            epc_paw = epc_paw +eepc*iatom(it)
         else
            if ( sw_remove_pcc_from_pawpot == ON ) then
                               ! Off means "consider pcc energy for only non-paw atoms"
               epc_paw = epc_paw +eepc*iatom(it)
            endif
         endif
#else
...
400   format( ' !PP ',' EPC     ENERGY = ',F20.12,' (HARTREE)'  )
    ...
    subroutine pcc
      ...
      do n = 1, nmesh(it)
         s   =  s + rhpcr(n)*wos(n)
      enddo
      if(ipripp >= 1) write(nfout,'(" !PP chgpc = ",d20.8)') chgpc(it)
      if(dabs(s-chgpc(it)) >= delta10) then
         if(ipripp >=1) write (nfout,'(" !PP partial core charge  s-chgpc= ",d20.8)') s-chgpc(it)
...
!!CDIR PARALLEL DO PRIVATE ( gabs, n, wkx, wky, fac )
  ...
  end subroutine m_PP_partial_core_CD

  subroutine m_PP_local_part(it,nfout,gr_l,ngshell,ngshell_range)
...
!!$         write(nfout,'(" !PP univol = ",f20.12)') univol
  ...
  end subroutine m_PP_local_part
...
1000  write(nfout,'(" eof is reached")')
   ...
   !================================================
      ...
      if ( intzaj == by_pseudo_atomic_orbitals ) then
         if ( is_gncpp == pp_PAW ) then
            call set_irank_minimal_pawpot( nfp, it, 0 )
         endif
      endif
...
!!$                do ia = 1, natm
!!$                   if(ityp(ia) /= it) cycle
!!$                   if(ia < ista_fs_atm .or. ia > iend_fs_atm) cycle
!!$                   if(abs(fq) < DELTA) cycle
!!$                   nac_p = nac_p + 1
!!$                end do
!!$             end do
...
!!$       allocate(nlmta1_p(nac_p), nlmta2_p(nac_p),fqwei_p(nac_p))
...
!!$       end do
  ...
  end subroutine m_PP_gfqwei

!=============================== added by K. Tagami ======================- 11.0
  subroutine m_PP_set_fqwei_noncl(nfout)
    ! --------------------------------------------------------
    integer, intent(in) :: nfout
    integer       :: i, ia, it, lmt1, lmt2
    integer :: is_tmp

    real(kind=DP) :: fq, qnorm
    nac = 0
    ...
    if(modnrm == SKIP) then
       ...
       fqwei_noncl = 0.d0; nlmta1 = 0; nlmta2 = 0; nac2ia = 0
...
! === ASMS 2017/08/31
!                   qnorm = qnorm + q_noncl( lmt1, lmt2, is_tmp, it ) &
!                        &        + conjg( q_noncl( lmt1, lmt2, is_tmp, it ) )
                   qnorm = qnorm + q_noncl( lmt1, lmt2, is_tmp, it ) &
                        &        * conjg( q_noncl( lmt1, lmt2, is_tmp, it ) )
! === ASMS 2017/08/31
                End do

                qnorm = sqrt( qnorm ) *iwei(ia)
                if( qnorm < DELTA) cycle
                ...
                Do is_tmp=1, ndim_chgpot
                   fqwei_noncl(nac, is_tmp)  = q_noncl( lmt1, lmt2, is_tmp, it ) &
                        &                      * iwei(ia)
                End Do
...
! === ASMS 2017/08/31
       ...
       if(.not.allocated(nlmta1_p)) allocate(nlmta1_p(nac_p))
       if(.not.allocated(nlmta2_p)) allocate(nlmta2_p(nac_p))
       if(.not.allocated(fqwei_p_noncl)) allocate(fqwei_p_noncl(nac_p,ndim_chgpot))
...
! === ASMS 2017/08/31
                ...
                Do is_tmp=1, ndim_chgpot
                   fqwei_p_noncl( i, is_tmp ) = q_noncl( lmt1, lmt2, is_tmp, it ) &
                        &                      * iwei(ia)
                End do
    ...
    if(ipripp >= 2) then
       ...
       write(nfout,'(" !PP --- iwei ---")')
       write(nfout,'(" !PP ",20i4)') (iwei(ia),ia=1,natm)
       write(nfout,'(" !PP --- m_PP_set_fqwei_noncl ---")')
       do it = 1, nac
          write(nfout,*) " !PP (nlmta1,nlmta2,nac2ia,fqwei_noncl)(",it,") = ",&
               & nlmta1(it),nlmta2(it),nac2ia(it),fqwei_noncl(it,1)
       end do
       ...
       write(nfout,'(" !PP --- m_PP_set_fqwei_noncl ---")')
       do it = 1, nac_p
          write(nfout,*) " !PP (nlmta1_p,nlmta2_p,fqwei_p_noncl)(",it,") = ",&
               & it,nlmta1_p(it),nlmta2_p(it),fqwei_p_noncl(it,1)
       end do
#ifndef FQWEI_CHECK
    end if
#endif

  end subroutine m_PP_set_fqwei_noncl
! ====================================================================== 11.0
