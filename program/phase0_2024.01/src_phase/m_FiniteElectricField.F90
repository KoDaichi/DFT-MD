...
!!$    write(nfout,'("i,mp_index=",2i6)') i,mp_index(i)
  ...
  subroutine calc_overlap(n1,n2,n3,ik0,iopr0,gs0,ik1,iopr1,gs1,ig,id)
    ...
    do m=1,ns
       ...
       call mpi_allreduce(mpi_in_place,smatt,ns,mpi_double_complex,mpi_sum,mpi_k_world(myrank_k),ierr)
       smat(:,m) = smatt(:)
    end do
...
!!    do i=1,ns
    ...
    call mpi_allreduce(mpi_in_place,p,1,mpi_double_complex,mpi_prod,mpi_k_world(myrank_k),ierr)
...
!end debug
  ...
  subroutine  calc_grad(n1,n2,n3,ik0,iopr0,gs0,m1,m2,m3,ik1,iopr1,gs1,ig,id)
    ...
    if(modnrm == EXECUT) then
      ...
      allocate(expgr(kg1));expgr = dcmplx(0.d0,0.d0)
    endif
    if(iopr0>nopr) then
       ipm0=-1
    else
    ...
    if(iopr0==1) then
       if(modnrm == EXECUT)then
          allocate(snlmat(iba(ik0),nlmta));snlmat=dcmplx(0.d0,0.d0)
          allocate(qfmat(nlmta,np_nvale));qfmat=dcmplx(0.d0,0.d0)
          allocate(cmat(iba(ik0),np_nvale));cmat=dcmplx(0.d0,0.d0)
          icount = 0
          do ia=1,natm
             it=ityp(ia)
             mdvdb = m_PP_include_vanderbilt_pot(it)
          ...
          enddo
          call zgemm('N','N',iba(ik0),ibcount,icount,cone,snlmat,iba(ik0),qfmat,nlmta,czero,cmat,iba(ik0))
          !do i=1,iba(ik0)
          !   do m=1,ibcount
          !      do lmt1=1,icount
          ...
          do m=ista_nvale,iend_nvale
             ...
             if(modnrm /= EXECUT) then
                ...
                do i=1,kgbpp(n1,n2,n3,id)
                   ii=indgbp(i,2,n1,n2,n3,id)
                   jj=indgbp(i,1,n1,n2,n3,id)
                   fmr = wfv(jj,mm,ik1,1) + dble(cmat(ii,ibcount))
                   fmi = wfv(jj,mm,ik1,2) * ipm1 + dimag(cmat(ii,ibcount))
                   grad(ii,1) = grad(ii,1) - sr*fmi - si*fmr
                   grad(ii,2) = grad(ii,2) + sr*fmr - si*fmi
                end do
             end if
    ...
    if(iopr1==1) then
       if(modnrm == EXECUT)then
          allocate(snlmat(iba(ik1),nlmta));snlmat=dcmplx(0.d0,0.d0)
          allocate(qfmat(nlmta,np_nvale));qfmat=dcmplx(0.d0,0.d0)
          allocate(cmat(iba(ik1),np_nvale));cmat=dcmplx(0.d0,0.d0)
          icount = 0
          do ia=1,natm
             it=ityp(ia)
             mdvdb = m_PP_include_vanderbilt_pot(it)
          ...
          enddo
          call zgemm('N','N',iba(ik1),ibcount,icount,cone,snlmat,iba(ik1),qfmat,nlmta,czero,cmat,iba(ik1))
          !do i=1,iba(ik1)
          !   do m=1,ibcount
          !      do lmt1=1,icount
          !         cmat(i,m) = cmat(i,m) + snlmat(i,lmt1)*qfmat(lmt1,m)
          ...
          do m=ista_nvale,iend_nvale
             ...
             if(modnrm /= EXECUT) then
                ...
                do i=1,kgbpm(m1,m2,m3,id)
                   ii=indgbm(i,2,m1,m2,m3,id)
                   jj=indgbm(i,1,m1,m2,m3,id)
                   fmr = wfv(jj,mm,ik0,1) + dble(cmat(ii,ibcount))
                   fmi = wfv(jj,mm,ik0,2) * ipm0 + dimag(cmat(ii,ibcount))
                   grad(ii,1) = grad(ii,1) + sr*fmi + si*fmr
                   grad(ii,2) = grad(ii,2) - sr*fmr + si*fmi
                end do
             end if
             ...
             if(modnrm /= EXECUT) then
                ...
                do ia=1,natm
                   ...
                   do lmt1=1,ilmt(it)
                      ...
                      qf = qf * zi**(-l1)
                      do i=1,iba(ik0)
                         ctmp = expgr(i)*snlt(i,lmt1,(ik0-1)/nspin+1)*qf
             ...
             if(modnrm /= EXECUT) then
                ...
                do ia=1,natm
                   ...
                   do lmt1=1,ilmt(it)
                      ...
                      qf = qf * zi**(-l1)
                      do i=1,iba(ik1)
                         ctmp = expgr(i)*snlt(i,lmt1,(ik1-1)/nspin+1)*qf
                         gsmat(i,1) = gsmat(i,1) + dble(ctmp)
                         gsmat(i,2) = gsmat(i,2) + dimag(ctmp)
                      end do
                   end do
...
! end debug
  ...
  subroutine m_FEF_add_grad_to_vnlph(ik)
    ...
    do ib=1,ns
       ...
       if(map_e(jb) == myrank_e) then
         call m_ES_add_it_to_vnlph(ik,map_z(jb),grad_fef(:,ib,ik,:))
       endif
    end do
  ...
  subroutine gather_valence_states(nfout)
    ...
    if(firstcall)then
      allocate(snlt(kg1,nlmtt,kv3/nspin));snlt=0.d0
      do iksnl=ista_snl,iend_snl
        snlt(1:kg1,1:nlmtt,iksnl) = snl(1:kg1,1:nlmtt,iksnl)
      enddo
      call mpi_allreduce(mpi_in_place,snlt,kg1*nlmtt*kv3/nspin,mpi_double_precision,mpi_sum,mpi_ge_world,ierr)
      firstcall = .false.
    endif
    ...
    do ik=1,kv3,af+1
       ...
       do ib=1,ns
          ...
          if(map_e(ib1) == myrank_e) then
             wfv(1:kg1,ib1,ik,1:kimg) = zaj_l(1:kg1,map_z(ib1),ik,1:kimg)
             neordr_val(ib,ik) = neordr(ib,ik)
             if(modnrm == EXECUT) then
                fsr_fef(ib1,1:nlmta,ik) = fsr_l(map_z(ib1),1:nlmta,ik)
                fsi_fef(ib1,1:nlmta,ik) = fsi_l(map_z(ib1),1:nlmta,ik)
             end if
          end if
       end do
    end do
    allocate(wfv_mpi(kg1,ns,kv3,kimg))
    allocate(neordr_mpi(ns,kv3))
    call mpi_allreduce(wfv,wfv_mpi,kg1*ns*kv3*kimg,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_CommGroup,ierr)
    call mpi_allreduce(neordr_val,neordr_mpi,ns*kv3,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_CommGroup,ierr)
    wfv = wfv_mpi
    neordr_val = neordr_mpi
    deallocate(wfv_mpi)
    deallocate(neordr_mpi)
    if(modnrm == EXECUT) then
