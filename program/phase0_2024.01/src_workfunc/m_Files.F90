...
! $Id: m_Files.F90,v 1.1.1.1 2004/06/26 11:21:37 yamasaki Exp $
  ...
  integer :: nfinp, nfout, nfcntn_bin, nfchgu, nfchgd, nfzaj, nfvlc, nfchgu_p, nfchgd_p, nfvlcr, nfvlcr_av
  data nfinp, nfout, nfcntn_bin, nfchgu, nfchgd, nfzaj, nfvlc, nfchgu_p, nfchgd_p, nfvlcr,nfvlcr_av  &
       & /31,   6,     55,         60,     61,    44,    45,     46,       47,       48,    49 /
...
!!$       &,  6,42,55,43,60,61,44,45 /
  logical :: nfinp_exists
  integer, parameter :: number_of_all_files = 11
...
!!$  end subroutine open_nfcntn
  ...
  subroutine m_Files_set_default_filenames
    F_file_names_data   = './wf_file_names.data'
...
!    F_CHGU      = './nfchgu.data'
!    F_CHGD      = './nfchgd.data'
!    F_CHGU_P    = './nfchgu_p.data'
!    F_CHGD_P    = './nfchgd_p.data'
    F_CHGU      = './nfchgu.cube'
    F_CHGD      = './nfchgd.cube'
    F_CHGU_P    = './nfchgu_p.cube'
    F_CHGD_P    = './nfchgd_p.cube'
...
1004    continue
  ...
  subroutine m_Files_open_files_initially
    inquire(file=F_INP,exist=nfinp_exists)
    if(nfinp_exists) call open0(nfinp, F_INP, 'F_INP     ',    old,   formatted,check_file_name_on)
...
!!$    call open0(nfcntn,F_CNTN,'F_CNTN    ',    old,   formatted,check_file_name_on)
    ...
    call open0(nfvlcr_av,F_VLCR_AV,'F_VLCR_AV ',unknown, formatted,check_file_name_on)
