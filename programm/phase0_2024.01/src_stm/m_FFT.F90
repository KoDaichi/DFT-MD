...
!!$    print *, ' nfftpf = ', nfftpf
  ...
  subroutine m_FFT_set_fft_box_size_fine
    if( kimg == 1 ) then
       fft_box_size_fine(1) = nlpf * 2
    else if ( kimg == 2) then
       fft_box_size_fine(1) = nlpf
    end if
  ...
  subroutine FFT_WF(afft,inverse_or_direct,switch)  ! G space --> R space
    ...
    integer :: id_sname = -1
    call tstatc0_begin('FFT_WF ',id_sname)
...
!!$    call FFT_CD(afft,inverse_or_direct)
  ...
  subroutine FFT_fine(afft,inverse_or_direct)   ! R space --> G space
    ...
    call tstatc0_begin('FFT_fine ',id_sname)
  ...
  subroutine FFT_CD(afft,inverse_or_direct)   ! R space --> G space
    ...
    call tstatc0_begin('FFT_CD ',id_sname)
...
!	INVERSE: 1, DIRECT: 2
    ...
    call tstatc0_begin('2dFFT_fine ',id_sname)
