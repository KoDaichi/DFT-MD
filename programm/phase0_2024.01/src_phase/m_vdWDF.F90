...
module m_vdWDF
  ...
  use m_Control_Parameters, only : printable,nspin,eval_kernel_by_interpolation, &
       ...
       &                           q0min, sw_use_WuGygi_method
  use m_Files, only : nfout
  use m_Charge_Density, only : m_CD_get_rspace_charge

  use m_FFT, only : fft_box_size_CD,fft_box_size_CD_nonpara, m_FFT_CD0,nfftp_nonpara
...
contains
  ...
  subroutine initialize_vdwdf_scf(nspin,ispin,na,nb,nc,chgr,grad_rho,version_no)
    use progress_bar, only : set_printable

    integer, intent(in) :: nspin,ispin,na,nb,nc, version_no
...
!!!    q0min = 0.09d0
  ...
  subroutine alloc_vdw()
    integer :: i,cix,ciy,ciz,itmp
    real(kind=DP) :: Ta,Tb,Tc, vec(3), bb(3,3)

    Allocate(rho(nabc));rho=0.d0
    allocate(grad(nabc));grad=0.d0
    Allocate(theta_G(nabc))
    Allocate(theta_R(nabc))
    if(.not.oneshot)then
       Allocate(dtheta_R(np_nq,nabc))
       Allocate(ddtheta_R(np_nq,nabc))
    endif
    Allocate(theta_G_a(nabc))
    Allocate(theta_G_b(nabc))
    ...
    if(sw_save_memory_vdw) then
       allocate(theta_G_ab(np_nq,nabc))
    else
       allocate(theta_G_ab(nq0,nabc))
    endif
    if(.not.oneshot)then
       !       allocate(ualpha_g(nq0,-(na/2-1):na/2,-(nb/2-1):nb/2,-(nc/2-1):nc/2))
       !       allocate(ualpha_r(nq0,na,nb,nc))
       allocate(dFdrho(nabc))
       allocate(dFddrho(nabc))
    endif
    ...
    Do ciz = -(nc/2-1),nc/2
       Do ciy = -(nb/2-1),nb/2
          Do cix = -(na/2-1),na/2
             rkar(get_index_1D(cix+na/2,ciy+nb/2,ciz+nc/2)) = &
       ...
       Do ciy = -(nb/2-1),nb/2
          Do cix = -(na/2-1),na/2
             vec(1:3) = dble(cix) *bb(1,1:3) &
                  &    +dble(ciy) *bb(2,1:3) &
                  &    +dble(ciz) *bb(3,1:3)
             rkar(get_index_1D(cix+na/2,ciy+nb/2,ciz+nc/2)) = DSQRT( vec(1)**2 +vec(2)**2 +vec(3)**2 )
          End Do
       End Do
    End Do
#endif
  ...
  subroutine build_theta()
    ...
    if(.not.oneshot)then
       allocate(tmpdr(nabc));tmpdr=0.d0
       allocate(tmpddr(nabc));tmpddr=0.d0
    ...
    Do cqa = ista_nq,iend_nq
       if(lprog) call progress()
       Call theta_ab(nabc,cqa,nq0,q0min,q0max,rho,grad,rhomin,theta_R,tmpdr,tmpddr)
  ...
  subroutine vdWdf_core_org()
    ...
    if(.not.oneshot)then
       allocate(tmpug(nabc));tmpug=0.d0
       allocate(tmpur(nabc));tmpur=0.d0
       !       ualpha_g = (0.d0,0.d0)
    else
       allocate(tmpug(1))
    endif
    ...
    if(.not.oneshot)then
       if(npes>1) then
          call mpi_allreduce(MPI_IN_PLACE,dFdrho, &
               &             nabc,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
  ...
  subroutine vdWdf_core()
    ...
    if(npes>1)then
       ...
       allocate(theta_buf_r(1:mp_nq,nabc));theta_buf_r=(0.d0,0.d0)
    endif
    ...
    do cqaa=1,mp_nq
       ...
       do i0=0,npes-1
          ...
          if(npes>1.and.i0.ne.npes-1)then
             ...
             allocate(theta_buf_s(1:mp_nq,nabc))
             theta_buf_s = theta_buf_r
             call mpi_sendrecv(theta_buf_s,mp_nq*nabc,mpi_double_complex,isend,0, &
                  &            theta_buf_r,mp_nq*nabc,mpi_double_complex,irecv,0, &
                  &            MPI_CommGroup,ista,ierr)
  ...
  subroutine vdWdf_core2()
    ...
    if(npes>1)then
       ...
       allocate(theta_buf_r(1:mp_nq,nabc));theta_buf_r=(0.d0,0.d0)
       if(.not.oneshot)then
          allocate(dtheta_buf(mp_nq,nabc));dtheta_buf=0.d0
       endif
    endif
    if(.not.oneshot)then
       allocate(tmpug(nabc));tmpug=0.d0
    ...
    do i0=0,npes-1
       ...
       if(npes>1)then
          call tstatc0_begin('vdWdf_core (comm) ',id_sname2,1)
          allocate(theta_buf_s(1:mp_nq,-(nabc/2-1):nabc/2))
          theta_buf_s = theta_buf_r
    ...
    if(.not.oneshot)then
       if(npes>1) then
          call mpi_allreduce(MPI_IN_PLACE,dFdrho, &
               &             na*nb*nc,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
          call mpi_allreduce(MPI_IN_PLACE,dFddrho, &
               &             na*nb*nc,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
          deallocate(tmpur)
       endif
...
! ----
  ...
  Subroutine piDphi(Ecii,Ecii_s)
    ...
    Enddo
    if(npes>1) call mpi_allreduce(MPI_IN_PLACE,Ecii,1,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
    ...
    Enddo
    if(npes>1) call mpi_allreduce(MPI_IN_PLACE,Ecii_s,1,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
  ...
  Subroutine cLDA(na,nb,nc,rho,rhomin,dv,EcLDA)
    ...
    Do cjr = 1,nabc
       ...
       EcLDA = EcLDA + dv*n*ec
    Enddo
    if(npes>1) call mpi_allreduce(MPI_IN_PLACE,EcLDA,1,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
  End Subroutine cLDA
  !** End SUBROUTINE cLDA **********************
  ...
  Subroutine RtoG(na,nb,nc,theta_R,theta_G)
    ...
    Real(kind=DP) theta_R(nabc)
    Complex(kind=DP) theta_G(nabc)
    ...
    allocate(temp_G(nabc))
    !***** FFT **************************************************
    temp_R(:) = DCMPLX(theta_R(:))
...
!    call dfftw_destroy_plan(plan)
    do ca = -(na/2-1),na/2
       do cb = -(nb/2-1),nb/2
          do cc = -(nc/2-1),nc/2
             ...
             ciy = MOD(cb+nb,nb)+1
             ciz = MOD(cc+nc,nc)+1
             theta_G(get_index_1D(ca+na/2,cb+nb/2,cc+nc/2)) = temp_G(get_index_1D(cix,ciy,ciz)) / DBLE(nabc)
          enddo
       enddo
    enddo
    !***** END of FFT ******************************************
  ...
  subroutine GtoR(na,nb,nc,theta_R,theta_G)
    !    include "fftw3.f"
    integer,intent(in) :: na,nb,nc
    complex(kind=DP), dimension(nabc), intent(in) :: theta_G
    complex(kind=DP), dimension(nabc), intent(out) :: theta_R

    integer :: cir,ca,cb,cc,cix,ciy,ciz
    integer(kind=DP) :: plan
    complex(kind=DP),allocatable :: temp_R(:),temp_G(:)
    real(kind=DP),allocatable,dimension(:) :: afft
    integer :: id_sname=-1
...
!    allocate(temp_R(nabc))
    ...
    call map_afft_to_rhog(afft,theta_R)
...
! ===============- For stress tensor =================
  subroutine initialize_vdwdf_stress( nspin, ispin, na, nb, nc, chgr, &
...
!!!    q0min = 0.09d0
    ...
    call alloc_vdw()
    allocate( cgrad(na*nb*nc,3)); cgrad=0.d0
    ...
    do cix = 1,na
       do ciy = 1,nb
          do ciz = 1,nc
             ...
             rho(cir2) = chgr(cir)
             grad(cir2) = grad_rho(cir)
             cgrad(cir2,1:3) = cgrad_rho(cir,1:3)
          enddo
       enddo
    enddo
  ...
  subroutine calc_stress_contrib2( na, nb, nc, cqa, cqb, nr12, phi_ab, &
    ...
    complex(kind=DP), intent(in)  :: theta_G_a(nabc)
    complex(kind=DP), intent(in)  :: theta_G_b(nabc)
    real(kind=DP),    intent(in)  :: phi_ab(0:nr12)
    real(kind=DP),    intent(in)  :: fac
    real(kind=DP), intent(inout) :: s_tensor(3,3)
    complex(kind=DP), intent(inout) :: tmpug(nabc)

    Integer  i, u, v
    Integer cir,cix,ciy,ciz,cr,ck
...
! ----
    ...
    Do ciz = -(nc/2-1),nc/2
       Do ciy = -(nb/2-1),nb/2
          Do cix = -(na/2-1),na/2
             cir = get_index_1D(cix+na/2,ciy+nb/2,ciz+nc/2)
             rk   = rkar(cir)
             ck   = dint(rk/dk)
             ...
             if ( ck > 0 ) then
                z1 = DCONJG( theta_G_a(cir) ) *theta_G_b(cir)  &
                     &                                *term2 /rkar(cir)
                Do v=1, 3
                   Do u=1, 3
  ...
  subroutine vdWdf_stress_tensor_core()
    ...
    if(npes>1)then
       allocate(ista(MPI_STATUS_SIZE))
       allocate(theta_buf_r(1:mp_nq,nabc));
  ...
  Subroutine convolution_3d_by_fft(na,nb,nc,cqa,cqb,nr12,phi_ab,theta_G_a,theta_G_b,fac,Ecnl_12_ab,tmpug)
    ...
    if(oneshot)then
       do cir = 1,nabc
          ...
          temp_c = temp_c + term *DCONJG(theta_G_a(cir))   &
               &          *              theta_G_b(cir)
       enddo
    else
       do cir = 1,nabc
