...
! $Id: WriteDownData_onto_Files_ek.f90 626 2020-06-03 02:18:38Z jkoga $
  ...
  use m_PseudoPotential,     only: m_PP_wd_PP_parameters, m_PP_wd_betar
  use m_PlaneWaveBasisSet,   only: kgp
  use m_NonLocal_Potential,  only: m_NLP_wd_snl
  use m_ES_LHXC,             only: m_ESlhxc_potential
  use m_ES_IO,               only: m_ESIO_wd_EigenValues_ek &
       &                         , m_ESIO_wd_WFs_and_EVS_ek, m_ESIO_wd_Eigenvalues_bxsf &
       &                         , m_ESIO_wd_Eigenvalues_frmsf
...
! =========================================================== 13.0E
  ...
  if(iconvergence_previous_job < EK_CONVERGED) then
     ...
     if(continuation_using_ppdata == YES) then
        ...
        call m_PP_wd_PP_parameters(nfout,nfcntn_bin,F_CNTN_BIN_partitioned,kgp)
        call m_PP_wd_betar(nfcntn_bin)
        call m_NLP_wd_snl(nfout,nfcntn_bin,F_CNTN_BIN_partitioned,kv3)
     end if
...
!!$        call m_ES_wd_eko(nfeng,mode=EK)
        ...
        if ( sw_write_bxsf_file == ON ) call m_ESIO_wd_EigenValues_bxsf
        if ( sw_write_bxsf_file == ON ) call m_ESIO_wd_EigenValues_frmsf
