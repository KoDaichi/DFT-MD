...
! =============== 2015/02/28
  ...
  subroutine angular_function(l,m,nq,qx,qy,qz,angfunc)
    ...
    if(l>=0) then
      ...
      call phase_error_with_msg(6,'Not supported for l<=-4',__LINE__,__FILE__)
    end if
...
! ====== KT_add ======== 2015/02/23
  subroutine m_Wan90_wd_eig_mpi(nfout)
    implicit none
    integer, intent(in) :: nfout

    integer :: ik,m,ib,mi, ik_start, iktmp
    real(kind=DP) :: ek
    ...
    if ( npes > 1 ) then
       allocate( eko_wk(  neg,kv3 ) );  eko_wk  = 0.0d0
...
!
       Do ik=1, kv3, ndim_spinor
          ...
          Do ib=1, neg
             ...
             eko_mpi( ib, ik ) = eko_l( map_z(ib), ik )
          End do
       End do
       call mpi_allreduce( eko_mpi, eko_wk, neg*kv3, &
    ...
    if ( mype == 0 ) then
       ...
       do ik = ik_start, kv3, nspin
          ...
          do m=1,num_bands
             mi = ib_inc(m)
             if ( npes > 1 ) then
                ek = eko_wk(mi,ik) *hartree
             else
                ib = neordr(mi,ik)
                ek = eko_l(map_z(ib),ik) * hartree
             endif
             write(nfwannier,'(2(1x,i5),1x,e25.16)') m, iktmp, ek
          end do
       end do
       close(nfwannier)
    endif
!
    if ( allocated(eko_wk) ) deallocate( eko_wk )

  end subroutine m_Wan90_wd_eig_mpi

  subroutine m_Wan90_gen_mmat_mpi(nfout)
    implicit none
    integer, intent(in) :: nfout

    real(kind=DP), allocatable :: m_mat(:,:,:,:,:) ! d(neg,neg,kv3,nntot,2)

#ifndef WAN90_SAVE_MEMORY
    allocate(m_mat(num_bands,num_bands,kv3,nntot,2)); m_mat = 0.d0
#else
...
!    call contrib_softpart
  ...
  contains
    ...
    subroutine contrib_hardpart
      ...
      integer :: ib1, ib2, ia, it, mdvdb
      integer :: lmt1, il1, im1, it1, lmta1
      integer :: lmt2, il2, im2, it2, lmta2
      integer :: ilm3, iiqitg, l3
      integer :: iktmp, ik_start, ik_skip, is1

      real(kind=DP) :: ylm, dk(3), c1
      complex(kind=CMPLDP) :: wf1, wf2, z1, z2, ztmp

      integer, allocatable :: il3(:)
      real(kind=DP), allocatable :: qx(:), qy(:), qz(:)
      complex(kind=CMPLDP), allocatable :: wk_fsri(:), zph(:)

! -- init
      n = nloc;   n=(n-1)+(n-1)+1
      allocate(il3(n**2));call substitute_il3(n**2,il3)
! ---
      allocate( qx(nntot), qy(nntot), qz(nntot) )
      Do nn=1, nntot
         qx(nn) = rltv(1,1) *dk_unit(1,nn) &
              & + rltv(1,2) *dk_unit(2,nn) &
              & + rltv(1,3) *dk_unit(3,nn)
         qy(nn) = rltv(2,1) *dk_unit(1,nn) &
              & + rltv(2,2) *dk_unit(2,nn) &
              & + rltv(2,3) *dk_unit(3,nn)
         qz(nn) = rltv(3,1) *dk_unit(1,nn) &
...
! -- start
      Do nn=1,nntot
         do ik1 = ik_start, kv3, ik_skip
            ...
            dk(1:3) = vkxyz(ik2,1:3,BUCS) +nnc(1:3) -vkxyz(ik1,1:3,BUCS)
!
            Do ia=1, natm
               c1 = PAI2 *dot_product( nnc, pos(ia,:) )
               zph(ia) = dcmplx( cos(c1), -sin(c1) )
            End Do

            myindex = 0
            mmloop : Do mm=1, nntot
               if ( abs(dk(1)-dk_unit(1,mm)) < Delta07 &
                    &  .and. abs(dk(2)-dk_unit(2,mm) ) < Delta07 &
                    &  .and. abs(dk(3)-dk_unit(3,mm) ) < Delta07 ) then
                  myindex = mm
                  exit mmloop
               endif
            end Do mmloop
            if ( myindex == 0 ) call phase_error_with_msg(nfout,"PPPP",__LINE__,__FILE__)
!
            do n=1,num_bands
               ni = ib_inc(n)
               ...
               call mpi_bcast( wk_fsri, 2*nlmta, mpi_double_precision, &
                    &          map_ek(ni,ik2), MPI_CommGroup, ierr )
                  ...
                  Do ia=1, natm
                     ...
                     Do lmt1=1, ilmt(it)
                        ...
                        Do lmt2=1, ilmt(it)
                           ...
                           ztmp = 0.0d0
                           do kk=1,il2p(lmt1,lmt2,it)
                              ilm3 = isph(lmt1,lmt2,kk,it); l3=il3(ilm3)
                              iiqitg = iqitg(il1,it1,il2,it2,l3+1,it)
                              if ( lmt1 > lmt2 ) then      ! ad hoc
                                 iiqitg = iqitg(il2,it2,il1,it1,l3+1,it)
                              endif

                              if(iiqitg == 0) cycle
                              call sphr( 1, ilm3, &
                                   &     qx(myindex), qy(myindex), qz(myindex), ylm )

                              z1 = conjg(wf1) *wf2 *(-zi)**l3 *dl2p(lmt1,lmt2,kk,it)
                              z2 = qitg_wan( iiqitg, myindex ) *ylm
                              ztmp = ztmp +z1 *z2 *zph( ia )
                           End do
                           m_mat(m,n,ik1,nn,1) = m_mat(m,n,ik1,nn,1) &
                                &              + real(ztmp)
                           m_mat(m,n,ik1,nn,2) = m_mat(m,n,ik1,nn,2) &
                                &              + aimag(ztmp)
                        End do
                     End do
                  End do
      ...
      deallocate( il3 )
      deallocate( zph );  deallocate( qx, qy, qz )

    end subroutine contrib_hardpart

    subroutine gather_matrix
      integer :: ik1
      integer :: isize, i, j, l, m
      integer :: ireqs1, ireqr1!, istatus1
      integer :: ireqs2, ireqr2, istatus2(MPI_STATUS_SIZE)
      ...
      if ( nrank_e > 1 ) then
         allocate(m_mat_mpi(num_bands,num_bands,ista_k:iend_k,nntot,2))
         m_mat_mpi = 0.0d0

         isize = num_bands *num_bands *(iend_k-ista_k+1) *nntot *2
         call mpi_allreduce( m_mat, m_mat_mpi, isize, mpi_double_precision, &
      ...
      if ( nrank_k > 1 ) then
         if ( mype == 0 ) then
            allocate(m_mat_mpi(num_bands,num_bands,kv3,nntot,2)); m_mat_mpi = 0.0d0
         endif
         allocate( wk(num_bands,num_bands,nntot,2) )
         isize = num_bands *num_bands *nntot *2

         Do ik1=1, kv3
            if ( map_ek(1,ik1) == mype .and. mype == 0 ) then
               m_mat_mpi(:,:,ik1,:,:) = m_mat(:,:,ik1,:,:)
            else
               if ( map_ek(1,ik1) == mype ) then
                  Do i=1, 2
                     Do j=1, nntot
                        Do l=1, num_bands
                           Do m=1, num_bands
                              wk(m,l,j,i) = m_mat(m,l,ik1,j,i)
                           End do
                        End do
                     End do
                  End do

                  call mpi_isend( wk, isize, mpi_double_precision, &
                       &          0, 0, MPI_CommGroup, ireqs2, ierr )
               endif
               if ( mype == 0 ) then
                  call mpi_irecv( wk, isize, mpi_double_precision, &
                       &          map_ek(1,ik1), 0, MPI_CommGroup, ireqr2, ierr )
               endif
               if ( map_ek(1,ik1) == mype ) call mpi_wait(ireqs2,istatus2,ierr)
               if ( mype == 0 )          call mpi_wait(ireqr2,istatus2,ierr)
               ...
               if ( mype == 0 ) then
                  Do i=1, 2
                     Do j=1, nntot
                        Do l=1, num_bands
                           Do m=1, num_bands
                              m_mat_mpi(m,l,ik1,j,i) = wk(m,l,j,i)
                           End do
                        End do
                     End do
                  End do
...
!
         if ( mype == 0 ) then
            deallocate( m_mat )
            allocate( m_mat(num_bands,num_bands,kv3,nntot,2)); m_mat = 0.0d0
            m_mat = m_mat_mpi
            deallocate( m_mat_mpi )
         endif
      endif
    ...
    subroutine print_mmat
      integer :: ik1, ik2, nn, n, m
      integer :: ik_start, iktmp, is1
      integer :: nnc(3)
      ...
      write(nfwannier,*) num_bands, kv3/nspin, nntot
      ...
      if ( noncol ) then
         do ik1 = 1, kv3, nspin
            ...
            do nn=1,nntot
               ik2 = nnlist(iktmp,nn)
               nnc(1:3) = nncell(1:3,iktmp,nn)
               write(nfwannier,'(5(1x,i5))') iktmp, ik2, nnc(1:3)
               do n=1,num_bands
                  do m=1,num_bands
                     write(nfwannier,'(2(1x,f16.12))') m_mat(m,n,ik1,nn,1:2) &
                          &                           +m_mat(m,n,ik1+1,nn,1:2)
                  end do
               end do
            end do
         end do
    ...
    end subroutine print_mmat

  end subroutine m_Wan90_gen_mmat_mpi

  subroutine m_Wan90_write_unk_mpi(nfout)
    implicit none
    integer, intent(in) :: nfout
    ...
    integer :: nd1, nd2, nd3
    integer :: i1, i2, i3
    integer :: ireqs, ireqr, istatus1(MPI_STATUS_SIZE)
    character(len=10) :: wfnname
    ...
    allocate(wf(nfft))
    if ( mype == 0 ) allocate(zwf(id1,id2,id3))
    ...
    do ispin = 1, nspin
       do iksnl = 1, kv3/nspin
          ...
          do n=1,num_bands
             ...
             if ( map_ek(ni,ik) == mype .and. mype == 0 ) then
                ib = neordr(ni,ik)
                call m_ES_WF_in_Rspace(ik,ib,wf)
             else
                if ( map_ek(ni,ik) == mype ) then
                   ib = neordr(ni,ik)
                   call m_ES_WF_in_Rspace(ik,ib,wf)
                   call mpi_isend( wf, nfft, mpi_double_precision, &
                        &          0, 0, MPI_CommGroup, ireqs, ierr )
                endif
                if ( mype == 0 ) then
                   call mpi_irecv( wf, nfft, mpi_double_precision, &
                        &          map_ek(ni,ik), 0, MPI_CommGroup, ireqr, ierr )
                endif

                if ( map_ek(ni,ik) == mype ) call mpi_wait( ireqs,istatus1,ierr )
                if ( mype == 0 ) call mpi_wait( ireqr,istatus1,ierr )
             endif
          ...
          if ( mype == 0 ) close(nfwannier)
       end do
    end do

    deallocate(wf)
    if ( allocated(zwf) ) deallocate(zwf)

  end subroutine m_Wan90_write_unk_mpi
! ============== 2015/02/23
