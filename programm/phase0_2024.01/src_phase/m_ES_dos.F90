...
!!$  use m_Files, only :                nfdos, nfout
  ...
  use m_Parallelization, only :      MPI_CommGroup,map_ek,mype,map_e,map_k,myrank_e,myrank_k &
       &                            ,ierr,np_e,map_z,ista_e,npes, nrank_e,ista_k,iend_k &
...
! =================================== added by K. Tagami ============ 11.0
  ...
  use m_Charge_Density, only :     chgq_l, chgq_enl, &
       ...
       &                           m_CD_restore_chgq, m_CD_keep_retrieve_hsr
...
! ================================================================ 11.0
  ...
  subroutine m_ESdos_put_dos_weight(ne,nk,dos_weight_from)
    ...
    if(.not.allocated(dos_weight)) &
         & call phase_error_with_msg(nfout, ' dos_weight is not allocated <<m_ESdos_put_dos_weight>>'&
         &                          , __LINE__, __FILE__)
    ...
    if(ekmode == ON) then
       if( ne /= neg .or. nk /= kv3_ek) then
          ...
          call phase_error_with_msg(nfout, ' ne /= neg or nk /= kv3_ek <<m_ESdos_put_dos_weight>>'&
               &                   , __LINE__, __FILE__)
       end if
    else if(ekmode == OFF) then
       if( ne /= neg .or. nk /= kv3) then
          ...
          call phase_error_with_msg(nfout, ' ne /= neg or nk /= kv3 <<m_ESdos_put_dos_weight>>' &
               &                   , __LINE__, __FILE__)
       end if
    end if
    ...
    if (ekmode == ON) then
       if ( ne /= neg .or. nk /= kv3_ek) then
          write(nfout,'(" !ldos ne /= neg or nk /= kv3_ek ",&
               ...
               &  ne,neg,nk,kv3_ek
          call phase_error_with_msg(nfout, ' ne /= neg or nk /= kv3_ek <<m_ESdos_put_dos_weight_noncl>>'&
               &                   , __LINE__, __FILE__)
       end if
       ...
       if ( ne /= neg .or. nk /= kv3) then
          write(nfout,'(" !ldos ne /= neg or nk /= kv3 ",&
               ...
               &    ne,neg,nk,kv3
          call phase_error_with_msg(nfout, ' ne /= neg or nk /= kv3 <<m_ESdos_put_dos_weight_noncl>>'&
               &                   , __LINE__, __FILE__)
       end if
    end if
...
! =============================================================== 11.0
    ...
    if(npes >=2 ) then
       call mpi_allreduce(mpi_in_place,eko,neg*kv,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
       if(sw_pdos == ON) then
          call mpi_allreduce(mpi_in_place,compr,neg*nlmta_phi*nopr*kv,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
          call mpi_allreduce(mpi_in_place,compi,neg*nlmta_phi*nopr*kv,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
          call mpi_allreduce(mpi_in_place,norm_phig_mpi,nlmtt_phi*kv/nspin,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
          norm_phig_mpi = norm_phig_mpi / dble(nrank_e) / dble(nrank_s)
       end if
    end if
    if(ipridos >= 2) then
       ! -- writing --
...
! =================================================================== 11.0
  ...
  subroutine pdos_ek()
    ...
    use m_ES_nonlocal, only : m_ES_betar_dot_WFs_4_each_k, m_ES_phir_dot_WFs
...
!!$#ifndef NO_TETRAHEDRON
    ...
    integer        :: neig,ispin,ip2,ik, ip, ib, jb, ie, nelm, iss, ies
...
!    real(kind=DP), allocatable, dimension(:,:) :: norm_phig_mpi
    ...
    integer ::                                  id_sname = -1
    integer :: i,iorb,iopr,ikorb,lmtt, ipridos_t, nEwindows_plus,mtetra,ikt,iloop, j
    ...
    if(ipridos_t >= 3) then
       ...
       if(npes >=2) then
          call mpi_allreduce(mpi_in_place,e_mpi,neg*kv3,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
       end if
       if(printable) then
          do ik = 1, kv3
...
!!$ASASASASAS
    ...
    if(mode == SCF) then
       if(npes >= 2) then
          call mpi_allreduce(MPI_IN_PLACE,eeig2,np2*neg*nspin,mpi_double_precision,mpi_sum &
               & , MPI_CommGroup,ierr)
          !eeig2 = eeig2_mpi
          !deallocate(eeig2_mpi)
          if(icomponent == TOTAL .and. sw_pdos == ON) then
            call mpi_allreduce(MPI_IN_PLACE,compr,neg*nlmta_phi*nopr*np2*nspin,mpi_double_precision,mpi_sum &
               & , MPI_CommGroup,ierr)
            call mpi_allreduce(MPI_IN_PLACE,compi,neg*nlmta_phi*nopr*np2*nspin,mpi_double_precision,mpi_sum &
               & , MPI_CommGroup,ierr)
                ...
                if (icomponent == TOTAL .and. sw_pdos == ON) then
                   ...
                   Do is1=1, ndim_spinor
                      ...
                      if ( k_symmetry(ik) /= GAMMA ) then
                         ...
                         call phase_error_with_msg(nfout, "Not supported : Gamma symmetry in noncollinear" &
                                                  , __LINE__, __FILE__)
                      endif
                   End do
                   norm_phig_mpi(1:nlmtt_phi,ip2) = norm_phig(1:nlmtt_phi,ip2)
    ...
    if (mode == SCF) then
       if (npes >= 2) then
          ...
          if (icomponent == TOTAL .and. sw_pdos == ON) then
            ...
            norm_phig_mpi = norm_phig_mpi2 / dble(nrank_e) / dble(nrank_s)
