...
! ----> work arrays of fft for wave functions
    ...
    if(kimg == 1) then
       ...
       allocate(cw1(nda3*2+np), stat=istat)
       allocate(cw2(nda3*2+np), stat=istat)
       allocate(cw3(nda3*2+np), stat=istat)
       if(ipri >= 1) write(nfout,'(" !! FFTE <<m_FFT.fft_WFCD_work_alloc>>")')
    end if
! ------- Positron start 
! ----> and work arrays of fft for positron wave functions
    if(sw_positron /= OFF) then
       ...
       if(kimg == 1) then
          ...
          allocate(cw1_pstrn(nda3/2+np), stat=istat)
          allocate(cw2_pstrn(nda3/2+np), stat=istat)
          allocate(cw3_pstrn(nda3/2+np), stat=istat)
       end if
    end if
! ------- Positron end
...
! ----> work arrays of ifft for the charge density
    if(kimg == 1) then
       ...
       allocate(wlp(nda3/2+np), stat=istat)
       allocate(wmp(nda3/2+np), stat=istat)
       allocate(wnp(nda3/2+np), stat=istat)
       if(ipri >= 1) write(nfout,'(" !! wlp,wmp,wnp are allocated, FFTE <<m_FFT.fft_WFCD_work_alloc>>")')
    end if
  ...
  subroutine m_FFT_setup(inversion_symmetry,paramset)
    ...
    if(inversion_symmetry == ON) then  ! kimg == 1
       ...
       ipad = 0
       if(ipri >= 1) write(nfout,*) '!FFT_WF = FFTE <<m_FFT_setup>>'
    end if
...
! Initialization of the Charge-Density FFT
  ...
  contains
    subroutine init_fft_coefficients_arrays_WF()
      ...
      if(kimg == 1) then
        ...
        call settbl(cw3,nn)
        if(sw_positron /= OFF) then
           call settbl(cw1_pstrn,nl_p)
           call settbl(cw2_pstrn,nm_p)
           call settbl(cw3_pstrn,nn_p)
        end if
      endif
  ...
  subroutine m_FFT_WF(electron_or_positron,nfout,afft,inverse_or_direct,switch)  ! G space --> R space
    ...
    if(electron_or_positron == ELECTRON) then
       ...
       if(kimg==1) id = fft_box_size_WF(1,0)
       nl = fft_box_size_WF(1,1)
       nm = fft_box_size_WF(2,1)
       nn = fft_box_size_WF(3,1)
       ...
       if(kimg==1) id = fft_box_size_pWF(1,0)
    ...
    if(kimg == 1) then
       ...
       if(inverse_or_direct == INVERSE) then
         isign = 2
       else if(inverse_or_direct == DIRECT) then
         isign = 1
       else
          if(ipri>=1) write(*,*) "inverse_or_direct must be 1 or 2 in subroutine m_FFT_pWF"
         stop
       endif
       call zfft3dw(afft,nl,nm,nn,cw1_t,cw2_t,cw3_t,isign)
    endif
  ...
  subroutine fft_CD_inverse_core(afft_CD)
    ...
    if(kimg == 1) then
       ...
       call zfft3dw(afft_CD,nlp,nmp,nnp,wlp,wmp,wnp,2)
    endif
  ...
  subroutine fft_CD_direct_core(afft_CD)
    ...
    if(kimg == 1) then
       ...
       call zfft3dw(afft_CD,nlp,nmp,nnp,wlp,wmp,wnp,1)
    endif
