...
! ==============================================================================
  ...
  use m_Parallelization,    only   : mpi_kg_world   &
       &                           , mpi_ke_world, mpi_chg_world &
       &                           , ista_atm_f, iend_atm_f
#endif
...
! === KT_add === 13.2S
  use m_Crystal_Structure,  only :  sw_use_magnetic_symmetry, &
       ...
       &                            Global_Quantz_Axis_Fixed, additional_charge
! ============== 13.2S
...
!!$         call flush(nfout)
         if(.not.prealloc) then
            ...
            if(tf) then
               ...
               if(0<rint) then
                  ...
                  if(ipriinputfile >=1)  write(nfout, &
                  '(" given iplane number is illegal. iplane = ",i5," <<set_constraint_atoms>>")') rint
    ...
    subroutine check_number_of_planes_atoms_are_fixed(nfcatm_rb,nfcatm_cog,num_planes_all,cnstrtype)
      ...
      if(icount < num_planes_atoms_are_fixed) then
         if(ipriinputfile >=1) write(nfout,'(" num_planes_atoms_are_fixed is reduced to ",i5," from ",i5)') &
         icount, num_planes_atoms_are_fixed
...
!        endif
  ...
  subroutine m_IS_scale_magmom0_atomtyp
    integer :: it
    real(kind=DP) :: c1
    ...
    if ( mag_moment0_atomtyp_is_defined ) then
       if ( mag_zeta1_atomtyp_is_defined ) then
          if ( noncol ) then
             Do it=1, ntyp
                c1 = ival(it) +qex(it) -additional_charge/dble(natm)
                mag_moment0_atomtyp(it,1:3) = mag_moment0_atomtyp(it,1:3) *c1
             End Do
          else
             Do it=1, ntyp
                c1 = ival(it) +qex(it) -additional_charge/dble(natm)
                mag_moment0_atomtyp(it,1) = mag_moment0_atomtyp(it,1) *c1
             End Do
          endif
       endif
    endif
  end subroutine m_IS_scale_magmom0_atomtyp
...
! -- Following 3 lines have been revised by Mamoru Usami, Oct. 2004. -->>
       if (mdalg .ne. VERLET) then
          ...
          call check_constraint(forc_l)  ! -> fcg_{cog|rb},fcg_mdfy_{cog|rb},tmass_{cog|rb}: modified group force along in a plane, tmass: group mass
    ...
    do ia = 1, natm
       if(imdtyp(ia)==FIX_IN_A_PLANE .or. imdtyp(ia)==COG_FIX_L .or. imdtyp(ia)==COG_FIX .or. &
          ...
          if(ip<=0 .or. nfcatm< ip) then
             ...
             call phase_error_with_msg(nfout, 'ip is out of range <<move_atoms_normal_to_plane>>',__LINE__,__FILE__)
          end if
          ...
          if((fcvect(ip,1)**2 + fcvect(ip,2)**2 + fcvect(ip,3)**2) < 1.d-9) then
             ...
             call phase_error_with_msg(nfout,' fcvect is too small <<move_atoms_normal_to_plane>>',__LINE__,__FILE__)
          end if
...
!!$             cpd_l(ia,:) = cpd_l(ia,:) - dcg_n(ip,:)
    ...
    if(nfcatm>=1 .and. iprimd >=DEBUGPRINTLEVEL ) then
       do ia = 1, natm
          if(imdtyp(ia) == COG_FIX_L .or. imdtyp(ia) == COG_FIX .or. imdtyp(ia) == RIGID_BODY_FIX_IN_A_PLANE) &
               & write(nfout,'(" cpd_l(",i4,") = ",3f13.6," corrected, <<correct_cog_andor_rb_motion>>")') ia, cpd_l(ia,:)
       end do
    end if
...
! further modified by T.Yamamoto on Nov. 29, 2005. --->>
          ...
          if(iprimd >= DEBUGPRINTLEVEL) write(nfout,'(i5,9f13.6)') ia, cpd_l(ia,1:3), frc(1:3), forc_l(ia,1:3)
...
!!$          cps(ia,1:3) = cps(ia,1:3) + fcvect(ip,1:3)*fcvect(ip,4)
          ...
          if(iprimd >= DEBUGPRINTLEVEL) then
             write(nfout,'(3i5,6f13.6," : ia, ip, ic, cps(",i4,",1:3), delta-cps")') &
!!$                  ia, ip, ic, cps(ia,1:3), fcvect(ip,1:3)*fcvect(ip,4), ia
                  ia, ip, ic, cps(ia,1:3), fcvect(ip,6:8)*fcvect(ip,4), ia
             call flush(nfout)
          end if
...
!!$    if (mdalg .ne. VERLET) return
  ...
  subroutine m_IS_structure_factor(nfout,kgp,ngabc)
    ...
    if(kimg == 1) then
#ifdef NEC_TUNE_SMP
       call structure_factor1(zfm3_l,pos,ngabc)
#else
       call structure_factor1
#endif
    else if(kimg == 2) then
#ifdef NEC_TUNE_SMP
       call structure_factor2(zfm3_l,pos,ngabc)
#else
  ...
  contains
#ifdef NEC_TUNE_SMP
    subroutine structure_factor1(zfm3_l,pos,ngabc)
#else
...
#ifdef NEC_TUNE_SMP
    subroutine structure_factor2(zfm3_l,pos,ngabc)
#else
...
! -----  ascat ceases modifying  -----
  ...
  end subroutine m_IS_structure_factor

  subroutine m_IS_ewald(nfout,kg,gr_l,kgp,ngabc,ival,alf_out)
...
! -----  ascat ceases modifying  -----
    ...
    if(sw_esm==OFF) then
       ...
       if(istress==1) call ewald_stress_Gspace_summation
        ...
        call mpi_allreduce(MPI_IN_PLACE,fxyzew_l,natm*3,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
    endif
#else
    call ewald_Gspace_summation
    if(istress==1) call ewald_stress_Gspace_summation
...
!!$       eewald = eewald + iwei(nu)*ival(ityp(nu))*etr
         ...
         do ia = 1, natm2
           if(ia == nu) cycle
           z = ival(ityp_full(ia))
           rnm1 = cps(nu,1) - cps(ia,1)
           rnm2 = cps(nu,2) - cps(ia,2)
           rnm3 = cps(nu,3) - cps(ia,3)
           rnms = rnm1*rnm1 + rnm2*rnm2 + rnm3*rnm3
           rnm  = dsqrt(rnms)
           rnmc = rnm*rnms
           etr  = etr + z/rnm
           fxyzew_l(nu,1) = fxyzew_l(nu,1) + rnm1*ival(ityp(nu))*z/rnmc
           fxyzew_l(nu,2) = fxyzew_l(nu,2) + rnm2*ival(ityp(nu))*z/rnmc
           fxyzew_l(nu,3) = fxyzew_l(nu,3) + rnm3*ival(ityp(nu))*z/rnmc
         end do
         eewald_rspace = eewald_rspace + iwei(nu)*ival(ityp(nu))*etr
end if
      end do
      ...
      if(npes > 1) then
         call mpi_allreduce(eewald_rspace, eewald, 1, mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
      else
    ...
    subroutine ewald_Rspace_summation_4_stress
      ...
      if(npes > 1) then
         allocate(s_ewt(3,3))
         call mpi_allreduce(s_ew,s_ewt,9,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
         s_ew = s_ewt
         deallocate(s_ewt)
      end if
    ...
    subroutine ewald_Gspace_summation
      ...
      if(npes > 1) then
         ...
         call mpi_allreduce(fxyzew_l, fxyzew_mpi, natm*3,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
         fxyzew_l = fxyzew_mpi
...
! -----  ascat starts modifying  -----
      ...
      if(npes > 1) then
         ...
         call mpi_allreduce(zsum, zsum_mpi, newldg*kimg,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
         zsum = zsum_mpi
         deallocate(zsum_mpi)
#ifdef __EDA__
         if(sw_eda==ON) then
...
! -----  ascat ceases modifying  -----
      ...
      if(npes > 1) then
         call mpi_allreduce(etr,etr_mpi,1,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
         etr = etr_mpi
      end if
...
!xocl end spread
    ...
    subroutine ewald_stress_Gspace_summation
      ...
      if( ist <= iend ) then
         do in = ist, iend  !for mpi
            ga = ngabc(in,1); gb = ngabc(in,2); gc = ngabc(in,3)
      ...
      if(npes > 1) then
         ...
         call mpi_allreduce(s_ewt,s_ew_mpi2,9,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
         s_ew = s_ew + s_ew_mpi2
...
!!$       print '(" ia, cps = ",i4,3d16.8)',ia,cps(ia,1),cps(ia,2),cps(ia,3)
  ...
  subroutine heatrsv_chain(mdalg,natm,nrsv,iw_cnst)
    ...
    if(.not.converged)then
      ...
      & 'dt/qmass to small or very unstable structure', &
      & __LINE__,__FILE__)
    endif
    cpd_l = cpd_t
    cpqr_c = cpqr_t
...
!!    rc2 = dftd3par%rcut_vdw**2
    ...
    enddo
    if(npes>1) then
       call mpi_allreduce(MPI_IN_PLACE,ncs,natm,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
       call mpi_allreduce(MPI_IN_PLACE,dncs,natm*3,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
       call mpi_allreduce(MPI_IN_PLACE,dncsdr,natm,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
    endif
    if(iprivdw>1) then
       do ia=1,natm
...
1000 continue
    ...
    if(npes>1) then
       ...
       call mpi_allreduce(MPI_IN_PLACE,forc,3*natm,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
       call mpi_allreduce(MPI_IN_PLACE,s_vdw,9,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
#ifdef __EDA__
       if ( sw_eda == ON ) then
...
! ============================================================= 13.0B
    ...
    if(npes > 1) then
       call mpi_allreduce(evdw, esum, 1, mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
       evdw = esum
       allocate(fxyzvdw_mpi(natm,3))
       call mpi_allreduce(fxyzvdw_l, fxyzvdw_mpi, natm*3,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
