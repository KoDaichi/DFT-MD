...
!  use m_Charge_Density,                   only  : chgq_l_EDA, chgq_l, hsr
  ...
  use m_Const_Parameters,                 only  : DIRECT, DP, EXC_ONLY, INVERSE, OFF, ON, Valence_plus_PC_Charge, ELECTRON
  use m_Control_Parameters,               only  : af, kimg, nspin
  use m_Crystal_Structure,                only  : altv, rltv, univol
  use m_Electronic_Structure,             only  : m_ES_WF_in_Rspace, m_ES_WF_kin_in_Rspace &
       &                                        , occup_l, totch
  use m_FFT,                              only  : m_FFT_alloc_WF_work, m_FFT_dealloc_WF_work &
       ...
       &                                        , m_FFT_WF &
       &                                        , fft_box_size_CD, fft_box_size_WF, nfft, nfftp, nfftp_nonpara
  ...
  use m_Parallelization,                  only  : ista_e, iend_e, istep_e &
       &                                        , ista_kngp, iend_kngp &
       &                                        , map_k, map_z,  myrank_k, np_e
  use m_PlaneWaveBasisSet,                only  : gr_l, iba, igf, igfp_l, kg, nbase, nbmx, ngabc
  use m_PseudoPotential,                  only  : dion, etot1, ilmt, ltp, mtp, psc_l &
       &                                        , epc_per_atom, PP_per_atom
  use m_XC_Potential,                     only  : exc_on_a_grid, m_XC_cal_potential
...
contains
  ...
  subroutine m_PW_EDA_allocation
    allocate(afft(nfftp))
    allocate(bfft1(nfftp))
    allocate(bfft2(nfftp))
...
!-------------------------------------------------------------------------------
    ...
    ifxr_CD = fft_box_size_CD(1,1)
    ifyr_CD = fft_box_size_CD(2,1)
    ifzr_CD = fft_box_size_CD(3,1)

    grid_weight_CD = 1.d0/(dble(product(fft_box_size_CD(1:3,1))))

    allocate(r_catom(natm2,3), cps_new(natm2,3), &
       &     grid_x_CD(ifxr_CD+1), grid_y_CD(ifyr_CD+1), grid_z_CD(ifzr_CD+1), &
       &     partition_function(ifxr_CD+1,ifyr_CD+1,ifzr_CD+1,natm2), partition_function_on_a_grid(natm2,nfftp))
...
!     call m_FFT_alloc_CD_box()
      ...
      call mpi_allreduce(mpi_in_place, afft, nfftp, mpi_double_precision &
...
!     deallocate(density_on_a_grid)
    ...
    subroutine calculate_kinetic_energy_per_atom
      ...
      integer                                    :: ispin, ib, ik, ifft, ri, iend, igp, i1, ierr
      real(kind=DP)                              :: fac
      integer                                    :: id_sname = -1
      call tstatc0_begin('calculate_kinetic_energy_per_atom ',id_sname,1)

      call m_FFT_alloc_WF_work()
      allocate(wf_EDA(nfft)); allocate(wf_kin_EDA(nfft))
...
!-----------------------------------------------------------------------
      ...
      do ispin = 1, nspin, af+1
         ekin_on_a_grid = 0.d0                ! ASMS add 2024/03/19

         do ib = ista_e, iend_e, istep_e                                 ! MPI
            do ik = ispin, kv3+ispin-nspin, nspin
               
               if(map_k(ik) /= myrank_k) cycle                               ! MPI
               call m_ES_WF_in_Rspace(ik,ib,wf_EDA)                          ! <- wavefunction
               call m_ES_WF_kin_in_Rspace(ik,ib,wf_kin_EDA)                  ! <- |k+G|^2*exp(-i(k+G)*r)
            ...
            enddo
         enddo
         call mpi_allreduce(mpi_in_place, ekin_on_a_grid, size(ekin_on_a_grid), mpi_double_precision, &
              &                  mpi_sum, MPI_CommGroup, ierr)
...
!-----------------------------------------------------------------------
      ...
      allocate(density_on_a_grid(nfftp)); density_on_a_grid = 0.d0
      allocate(ehartr_on_a_grid2(nfftp)); ehartr_on_a_grid2 = 0.d0
      call m_FFT_alloc_CD_box()
      ...
      call mpi_allreduce(mpi_in_place, density_on_a_grid, nfftp, mpi_double_precision &
      &                 ,mpi_sum, MPI_CommGroup, ierr)
      call mpi_allreduce(mpi_in_place, ehartr_on_a_grid2, nfftp, mpi_double_precision &
      &                 ,mpi_sum, MPI_CommGroup, ierr)
...
!     call m_FFT_CD_direct0(nfout,ehartr_on_a_grid2)
      ...
      allocate(ehartr_on_a_grid(nfftp)); ehartr_on_a_grid = 0.d0
...
!-----------------------------------------------------------------------
      ...
      integer                                    :: ri, ip, igp, ispin, it, iatm, i
      integer, parameter                         :: UP = 1, DOWN = 2
      integer                                    :: ierr
      integer                                    :: id_sname = -1
...
!-----------------------------------------------------------------------
      ...
      allocate(elocal_on_a_grid1(nfftp)); elocal_on_a_grid1 = 0.d0
      call m_FFT_alloc_CD_box()
      ...
      call mpi_allreduce(mpi_in_place, elocal_on_a_grid1, nfftp, mpi_double_precision &
      &                 ,mpi_sum, MPI_CommGroup, ierr)
...
!-----------------------------------------------------------------------
      ...
      allocate(PP_on_a_grid_wk(nfftp)); PP_on_a_grid_wk = 0.d0

! ---> ASMS mod 2024/03/19
!!!      allocate(PP_on_a_grid(nfftp,natm2)); PP_on_a_grid = 0.d0
...
!        call m_FFT_CD_inverse_ascat(nfout,PP_on_a_grid_wk)
        call mpi_allreduce(mpi_in_place, PP_on_a_grid_wk, nfftp, mpi_double_precision &
        &                 ,mpi_sum, MPI_CommGroup, ierr)
...
! <--- ASMS mod 2024/03/19
      ...
      allocate(elocal_on_a_grid(nfftp)); elocal_on_a_grid = 0.d0
      ...
      &                 elocal_from_nucleus2/dble(product(fft_box_size_CD(1:3,1)))
...
!------------------------------------------------------------------------------
    ...
    subroutine partition_into_atoms(afft_on_a_grid,afft_per_atom)
      ...
      real(kind=DP),intent(in),dimension(nfftp)  :: afft_on_a_grid
