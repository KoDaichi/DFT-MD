...
! $Id: Initial_Electronic_Structure.F90 633 2020-12-01 05:11:03Z jkoga $
  ...
  use m_Control_Parameters, only : ipri, iprichargedensity, initial_chg, icond, nspin, intzaj &
       ...
       &                         , sw_pdos, sw_precalculate_phase_vnonlocal
...
#else
       &                         , m_ES_EXX_update, m_ES_EXX_crotylm
#endif
...
! ================================== added by K. Tagami =============== 11.0
  use m_Const_Parameters,        only : ByPawPot, ZeffApprox, ReadFromPP
...
! ===== KT_add ======= 13.0U2&13.0U3
  use m_Control_Parameters,  only : sw_potential_mixing, &
       &                            sw_modified_TFW_functional, &
       &                            sw_spherical_averaged_nonlocal, &
       &                            use_averaged_nonlocal, sw_wf_mixing
  use m_Potential_Mixing,    only : m_Pot_alloc_array_vnonlocal
  use m_ThomasFermiW_Potential, only : m_TFW_normalize_psir_val, &
       &                               m_TFW_alloc_ChgDensityBasisFn_A, &
       &                               m_TFW_alloc_ChgDensityBasisFn_B, &
       &                               m_TFW_set_ChgDensityBasisFn_A, &
       &                               m_TFW_set_ChgDensityBasisFn_B, &
       &                               m_TFW_alloc_vnonlocal_avg, &
       &                               m_TFW_alloc_Vin_and_Vlda
  use m_ES_WF_mixing,      only :      m_ES_WF_alloc_zaj_prev
! ==================== 13.0U2&13.0U3
  use m_ES_RSB,            only : m_ES_RSB_alloc,m_ES_RSB_doit,m_ES_RSB_dealloc

! ==== KT_add  ==== 2014/08/29
  use m_Control_Parameters,  only : sw_use_add_proj, Orbital_decomp_mode, &
       &                            sw_use_contracted_psir
  use m_Charge_Density,  only : m_CD_alloc_hsr_add
  use m_CD_Mag_Moment,   only : m_CD_calc_product_psir_val, m_CD_alloc_product_psir_val
! ================= 2014/08/29
! ==== KT_add  ==== 2014/09/19
  use m_Control_Parameters,  only : sw_calc_ekin_density, use_symm_ekin_density, &
       &                            sw_rspace_ekin_density, use_asymm_ekin_density
  use m_KineticEnergy_Density,     only : m_KE_calc_kins_qrs_rspace, &
       &                                  m_KE_calc_kina_qrs_rspace, m_KE_init_ekindens
! ================= 2014/09/19
...
! ==== KT_add === 2014/08/29
     if ( sw_use_add_proj == ON .and. Orbital_decomp_mode == 2 ) then
        call m_CD_alloc_hsr_add()
     endif
     if ( sw_use_contracted_psir == ON ) then
        call m_CD_alloc_product_psir_val
     endif
! =============== 2014/08/29

! ==== KT_add ======= 13.0U2
     if ( sw_potential_mixing == ON ) call m_Pot_alloc_array_vnonlocal
! =================== 13.0U2
...
! ========= KT_add ========== 13.0U3
  if ( sw_wf_mixing == ON ) call m_ES_WF_alloc_zaj_prev
! =========================== 13.0U3
...
! ==== KT_add === 2014/09/19
     if ( sw_calc_ekin_density == ON .and. sw_rspace_ekin_density == ON ) then
        if ( use_symm_ekin_density )  call m_KE_calc_kins_qrs_rspace
        if ( use_asymm_ekin_density ) call m_KE_calc_kina_qrs_rspace
     endif
! =============== 2014/09/19
! ==== KT_add === 2014/08/29
     if ( sw_use_contracted_psir == ON ) then
        call m_CD_calc_product_psir_val
     endif
! =============== 2014/08/29
! ===================================== added by K. Tagami ============ 11.0
     if ( noncol ) then
...
! ==============================================================================
  ...
  if(icond == INITIAL .or. icond == COORDINATE_CONTINUATION .or. &
     ...
     if(.not.is_charge_density_read.and.(initial_chg == FILE .or. (icond==FIXED_CHARGE.and.ekmode==OFF))) then
        ...
        if ((iteration_unit_cell > 1.or. iteration_stress_correction>1) .and. sw_read_nfchgt_prev_cell == ON ) then
           write(6,'(" -- reading charge density (Initial_Electronic_Structure)")')
           call flush(6)
     ...
     if ( sw_opencore == ON ) then
        ...
        if ( sw_eval_epc_on_fftmesh == ON ) then
           call m_XC_cal_potential( nfout, Partial_Core_Charge, chgq_l, EXC_ONLY )
           epc = exc
        endif
     endif
...
!
...
#ifdef _CUDA_
  subroutine send_data_to_device()
    use m_Electronic_Structure, only : snl
    use m_PlaneWaveBasisSet,    only : ngabc, nbase
    use m_PseudoPotential,      only : q, dion, dion_paw, ilmt, lmtt, lmta, ltp, mtp, ipaw
    use m_Ionic_System,         only : ityp
    !$acc enter data copyin(snl, ngabc, nbase, q, dion, dion_paw, ityp, ilmt, lmtt, lmta, ltp, mtp, ipaw)
  end subroutine send_data_to_device
#endif
  ...
  subroutine EXX()
    ...
    if(sw_rsb==ON)then
       call m_ES_RSB_alloc()
       call m_ES_RSB_doit(verbose=.false.,fermi=.false.)
    endif
...
!                           @(#)Initial_Electronic_Structure.f90 1.4 03/02/19 00:43:18
  ...
  use m_Electronic_Structure,only: m_ES_energy_eigen_values &
       &                         , m_ES_cpeko &
       &                         , m_ES_wd_zaj_small_portion0, snl
  use m_ES_nonlocal,        only : m_ES_betar_dot_WFs, m_ES_AtaulmnaG
  use m_ES_ortho,           only : m_ES_modified_gram_schmidt
  use m_ES_initialWF,       only : m_ESIW_by_randomnumbers &
