...
***/
/******************************************************
PHASE-System input-file parse tool functions
...
#define HIT  1
#define NOHIT 0
int iflag_duplicated_block;
char duplicated_block_name[MAXTAGLEN+1];

/* int check_input_duplicated_tag_(); */
...
double str_to_double(char line[], int *ierr) {

    if(line=="pi")
        return M_PI;

    *ierr = 1;
    char *hatpos_ptr = strchr(line,'^');
    int hatpos = (hatpos_ptr == NULL ? -1 : hatpos_ptr-line);
    if (hatpos>0) {
        char basechar[256]="";
        char powchar[256]="";
        memcpy(basechar,&line[0],hatpos);
        memcpy(powchar, &line[hatpos+1],strlen(line));
        if(is_realstr(basechar)==0)
            *ierr = 0;
            
        if(is_realstr(powchar)==0)
            *ierr = 0;
        double base = atof(basechar);
        double pw = atof(powchar);
        return pow(base,pw);
    }

    char *delim_ptr = strchr(line,'(');
    int delimpos = (delim_ptr == NULL ? -1 : delim_ptr-line);
    char *delime_ptr = strchr(line,')');
    int delimepos = (delime_ptr == NULL ? -1 : delime_ptr-line);
    if(delimpos>0 && delimepos>0) {
        char token[6] = "";
        char arg[20]  = "";
        double farg;
        memcpy(token,&line[0],delimpos);

        if (strncmp(token,"log10",5)==0) {
            memcpy(arg,&line[6],get_last_index(line)-strlen(token)-2);
            if(is_realstr(arg)==0)
                *ierr = 0;
            farg = atof(arg);
            return log10(farg);
        }
        if (strncmp(token,"+log10",6)==0) {
            memcpy(arg,&line[7],get_last_index(line)-strlen(token)-2);
            if(is_realstr(arg)==0)
                *ierr = 0;
            farg = atof(arg);
            return log10(farg);
        ...
        if (strncmp(token,"cos",3)==0) {
            memcpy(arg,&line[4],get_last_index(line)-strlen(token)-2);
            if(is_realstr(arg)==0)
                *ierr = 0;
            farg = atof(arg);
            return cos(farg*M_PI/180.0);
        }
        if (strncmp(token,"+cos",4)==0) {
            memcpy(arg,&line[5],get_last_index(line)-strlen(token)-2);
            if(is_realstr(arg)==0)
                *ierr = 0;
            farg = atof(arg);
            return cos(farg*M_PI/180.0);
        ...
        if (strncmp(token,"sin",3)==0) {
            ...
            if(is_realstr(arg)==0)
                *ierr = 0;
            farg = atof(arg);
            return sin(farg*M_PI/180.0);
        }
        if (strncmp(token,"+sin",4)==0) {
            memcpy(arg,&line[5],get_last_index(line)-strlen(token)-2);
            if(is_realstr(arg)==0)
                *ierr = 0;
            farg = atof(arg);
        ...
        if (strncmp(token,"tan",3)==0) {
            memcpy(arg,&line[4],get_last_index(line)-strlen(token)-2);
            if(is_realstr(arg)==0)
                *ierr = 0;
            farg = atof(arg);
            return tan(farg*M_PI/180.0);
        }
        if (strncmp(token,"+tan",4)==0) {
            memcpy(arg,&line[5],get_last_index(line)-strlen(token)-2);
            if(is_realstr(arg)==0)
                *ierr = 0;
            farg = atof(arg);
	...
	for( i = 0; i < n; i++ ){
		if( strcmp( wktag, binfo->childtags[i] ) == 0 ){
			...
			if( wkinfo.tblinfo != NULL ){
				...
				printf("In set_bigtable: Syntax Error: duplicated block. \"%s\"\n", wktag);
			   /* printf("In set_bigtable: %d th '%s' block has no '%s' indication.\n",cnt,wktag,TABTAGSTR); */
			  iflag_duplicated_block = HIT;
			  strcpy( duplicated_block_name, wktag );
			     /* printf("Syntax Error: duplicated block(2). \"%s\"\n", duplicated_block_name); */
...
**/
	...
	if( (blk_info=(BLKINFO *)malloc( sizeof(BLKINFO)*MAXBLKDEPTH )) == NULL ){
		printf("input file: blockinfo malloc error, size=[%ld]\n", sizeof(BLKINFO)*MAXBLKDEPTH);
		free( rbuf );
		return( -1 );
	}
	...
	if( (bigtable=(BIGTABLE_INFO *)malloc( sizeof(BIGTABLE_INFO) )) == NULL ){
		printf("input file: bigtable malloc error, size=[%ld]\n", sizeof(BIGTABLE_INFO));
		free( rbuf );
		free( blk_info );
		return( -1 );
	}
...
/**/
	...
	if( (ret=set_childtaginfo( &blk_info[blk_depth] )) < 0 ){
		printf("In selectBlock: set_childtaginfo=%d\n",ret);
		blk_depth--;
		return( eSYNTAX);
	}
	if( set_blockunits( &blk_info[blk_depth] ) < 0 ){
...
/* resolve binary operations */
        ...
        if (iflg==0 && wk[0] != 45 && strchr(wk,100) == NULL && strchr(wk,101) == NULL) {
            tp = strtok(wk,"-");
            r0 = atof(tp);
            r1 = 0.0;
            while ( tp != NULL ) {
        ...
        if (iflg==0 && wk[0] != 43 && strchr(wk,100) == NULL && strchr(wk,101) == NULL) {
            tp = strtok(wk,"+");
            r0 = atof(tp);
            r1 = 0.0;
            while ( tp != NULL ) {
...
/* resolve binary operations */
        ...
        if (iflg==0 && wbuf[0] != 45 && strchr(wbuf,100) == NULL && strchr(wbuf,101) == NULL) {
            tp = strtok(wbuf,"-");
            r0 = str_to_double(tp,&ierr);
            if(ierr==0) {
                *ret = 0.0;
        ...
        if (iflg==0 && wbuf[0] != 43 && strchr(wbuf,100) == NULL && strchr(wbuf,101) == NULL) {
            tp = strtok(wbuf,"+");
            r0 = str_to_double(tp,&ierr);
            if(ierr==0) {
                *ret = 0.0;
...
//		return( eINVALIDVAL );
//	}
	if(iflg ==0) wkret = str_to_double( wbuf, &ierr );
        if(ierr==0) {
            *ret = 0.0;
            errout( tag, wbuf );
            return( eINVALIDVAL );
...
int check_input_duplicated_tag_( )
{
  return( iflag_duplicated_block );
}

void cp_duplicated_block_name_( char *tag)
{
  int i;
  strcpy( tag, duplicated_block_name);
  /*  for(i=0;i<=sizeof(duplicated_block_name)-1;i++){
    if(duplicated_block_name[i]=='\0') duplicated_block_name[i]=' ';
  }*/
  
  /*  duplicated_block_name[sizeof(duplicated_block_name)-1]=' ';*/
}
