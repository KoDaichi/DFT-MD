...
! <--
     if(ChargeDensity_is_Converged) then
        ...
        if(nspin == 2 .and. sw_fix_total_spin_in == YES .and. spin_fix_period >= INITIALLY) then
           call m_CS_reset_fixed_total_spin(nfout) ! 2024.03.27 T. Yamasaki
           total_spin_new = m_ESoc_new_total_spin()
           call m_CS_set_total_spin(nfout,total_spin_new)
        end if
#endif
...
logical function EigenValues_are_Converged()
  ...
  use m_ES_IO,                only : m_ESIO_wd_EigenValues, m_ESIO_wd_WFs_and_EVs_ek
...
subroutine Postproc_after_SCF_convergence(nfout)
  ...
  subroutine change_gmax()
    ...
    & , m_pwBS_generate_G_vectors_3D, m_pwBS_set_ngabc_kngp_l_3D &
    & , m_pwBS_alloc_ngpt_igfp_gr_3D, m_pwBS_calc_length_of_G_3D &
    & , m_pwBS_G_trans_functions_3D, m_pwBS_setup_FFTmapfunctions_3D &
    & , n_rGv,n_rGpv,n_rGpv_reduced, n_rGv_pstrn,kgp, n_rGv_prev &
    & , n_rGpv_prev, kgp, m_pwBS_rd_prev_pws, m_pwBS_for_each_WF &
    & , m_pwBS_alloc_ylm_l, m_pwBS_sphrp_l_3D, pre_charge_average_3D
    use m_Parallelization,    only : m_Parallel_init_mpi_kngp_prev_3D &
    & , m_Parallel_store_prev_np_g1k, np_g1k, m_Parallel_init_mpi_kngp_3D &
    & , m_Parallel_init_mpi_cdfft
    use m_PseudoPotential,    only : m_PP_input_xctype, modnrm, m_PP_dealloc, m_PP_gfqwei_3D, m_PP_init_epc
    use m_FFT,                only : m_FFT_set_box_sizes, m_FFT_setup &
    & , m_FFT_reset_CD_setup_stat, m_FFT_reset_firstcall
    use m_Electronic_Structure, only : m_ES_alloc_zaj_l_prev &
    & , m_ES_dealloc_zaj_l_prev, m_ES_cp_zaj_to_zaj_prev  &
    & , m_ES_cp_zaj_to_zaj_prev, m_ES_gen_zaj_from_prev_zaj, m_ES_dealloc, m_ES_alloc_zaj_etc, m_ES_realloc_zaj &
    ...
    call m_pwBS_generate_G_vectors_3D()
    call m_Parallel_init_mpi_kngp_3D(nfout, ipriparallel,kgp, sw_communicator_for_chg==ON)
  ...
  subroutine pwbs_mpi_initialization()
    ...
    use m_Control_Parameters, only : nblocksize_mgs, nblocksize_mgs_is_given, neg, ipriparallel &
    &                       ,        sw_fft_xzy, fftbox_divide_cube, fftbox_div_1, fftbox_div_2 &
    &                       ,        flag_mpi_g_dot_r, flag_mpi_g_dot_r_k, sw_communicator_for_chg &
    &                       ,        ngnode_nbmx
    use m_FFT, only : nfft, nfftp, nfftps
    ...
    call m_Parallel_dealloc_mpi_gga()
    call m_Parallel_dealloc_mpi_mix()
   ...
   &                               fftbox_div_1,fftbox_div_2)
    ...
    if(ekmode == OFF) then
       call m_Parallel_wf_onto_fft_dealloc_3D()
    ...
    call m_Parallel_fft_onto_chgq_dealloc()
    call m_Parallel_chgq_onto_fftcd_dealloc()
    call m_Parallel_fftcd_onto_chgq_dealloc()
