...
!!$                                  &, EXC_ONLY,STRESS_, CRDTYP, GAMMA
  ...
  use m_PlaneWaveBasisSet,    only : kg1,kg,kgp,ngabc,iba,nbase,nbmx,gr_l,ylm_l &
                                  &, m_pwBS_sphrp2_3D,m_pwBS_sphrp2_diff_3D &
                                  &, ngabc_kngp_l
  use m_Electronic_Structure, only : zaj_l,occup_l,eko_l,totch &
                                  &, fsr_l,fsi_l,fsr_l_2D,fsi_l_2D &
                                  &, fsr_diff_l_2D,fsi_diff_l_2D &
                                  &, m_ES_alloc_fsri_l_2D, m_ES_dealloc_fsri_l_2D &
                                  &, m_ES_gather_f_3d_to_2d, m_ES_alloc_fsr_l_2d &
                                  &, m_ES_alloc_fsi_l_2d, m_ES_gather_f_3d_to_2d_blk &
                                  &, m_ES_dealloc_fsr_l_2d, m_ES_dealloc_fsi_l_2d &
                                  &, m_ES_alloc_fsr_diff_l_2d, m_ES_alloc_fsi_diff_l_2d &
                                  &, m_ES_dealloc_fsr_diff_l_2d, m_ES_dealloc_fsi_diff_l_2d

  use m_ES_nonlocal,          only :                                            &
 &                                   m_ES_betar_dot_WFs_4_lmta_k_3D             &
 &                                 , G_dot_R                                    &
 &                                 , m_ES_alloc_zfsincos, m_ES_dealloc_zfsincos &
 &                                 , m_ES_alloc_arai_3D, m_ES_dealloc_arai_3D   &
 &                                 , alloc_zfsincos_mpi, dealloc_zfsincos_mpi   &
 &                                 , m_ES_betar_dot_WFs_4_each_k_3D
#ifdef NONLOCAL_DGEMM
  use m_ES_nonlocal,          only : m_ES_betar_dot_Psi_k_3D_snl
#endif
 ...
 &                                 , ke_e0,ke_sigma,ke_a, xctype, nblocksize_gather_f_is_given, nblocksize_gather_f &
 &                                 , m_CtrlP_cachesize
  ...
  use m_Parallelization,      only : MPI_CommGroup,map_k,myrank_k,np_e,ierr,ista_k,iend_k &
                                  ...
                                  &, ista_g1k, iend_g1k, np_g1k, mpi_ke_world, mpi_kg_world  &
                                  &, mpi_ge_world,nrank_g, mpi_chg_world, ista_atm_ke, iend_atm_ke
...
! =================================================================== p1100
  ...
  real(kind=DP),dimension(3,3),private :: currstress
...
! ================================================================== p1100
  ...
  subroutine m_Stress_ke(nfout,kv3,kp_crdtyp,vkxyz)
    ...
    allocate(g(maxval(np_g1k),3)); g = 0.d0
...
!!$ASASASASAS
    integer         :: lnblck, lnb1, lnb2, lnsize,ib
...
!!$ASASASASAS
    ...
    do ispin = 1, nspin, af+1
       do ik = ispin, kv3+ispin-nspin, nspin
          ...
          call m_ES_betar_dot_Psi_k_3D_snl &
          ...
          call m_ES_alloc_fsr_diff_l_2d(lnblck, nlmta)
          call m_ES_alloc_fsi_diff_l_2d(lnblck, nlmta)
          do lnb1 = 1, np_e, lnblck                          ! MPI
             ...
             call m_ES_gather_f_3d_to_2d_blk(fsr_diff_l, fsr_diff_l_2D, ik, lnblck, lnb1, lnsize)
             call m_ES_gather_f_3d_to_2d_blk(fsi_diff_l, fsi_diff_l_2D, ik, lnblck, lnb1, lnsize)
             do ia = 1, natm
               it = ityp(ia)
               do ib=lnb1,lnb2
               w_n = occup_l(ib,ik) * fac
               ek_n = eko_l(ib,ik)
               do lmt1 = 1,ilmt(it)
                 lmta1    = lmta(lmt1,ia)
                 do lmt2 = lmt1,ilmt(it)
                   lmta2    = lmta(lmt2,ia)
                   if (k_symmetry(ik) /= GAMMA) then
                     dtmp1 = fsi_l_2D(ib,lmta2)
                     dtmp2 = fsi_l_2D(ib,lmta1)
                   else
                     dtmp1 = 0.d0
                     dtmp2 = 0.d0
                   endif
                   c= w_n * &
                    &     (  fsr_diff_l_2D(ib,lmta1)*fsr_l_2D(ib,lmta2) &
                    &     +  fsi_diff_l_2D(ib,lmta1)*dtmp1 &
                    &     +  fsr_diff_l_2D(ib,lmta2)*fsr_l_2D(ib,lmta1) &
                    &     +  fsi_diff_l_2D(ib,lmta2)*dtmp2 )
          ...
          call m_ES_dealloc_fsr_diff_l_2d()
          call m_ES_dealloc_fsi_diff_l_2d()
       enddo
    enddo
#else
      do ia = 1, natm
...
!!$ASASASASAS
        ...
        do ik = ispin, kv3+ispin-nspin, nspin
          if(map_k(ik) /= myrank_k) cycle         ! MPI
!          call m_ES_betar_dot_WFs_4_each_k_3D(nfout,ik,.false.)
...
!             if(.not. allocated(fsi_l_2D)) allocate(fsi_l_2D(np_e, nlmta))
...
!          endif
          ...
          call m_ES_betar_dot_WFs_4_lmta_k_3D &
!!$ASASASASAS
...
!!$                  &     +  fsi_diff_l(i,lmta2,ik)*fsi_l(i,lmta1,ik) )
                 c= w_n * &
                  &     (  fsr_diff_l(ib,lmta1,ik)*fsr_l_2D(ib,lmta2) &
                  &     +  fsi_diff_l(ib,lmta1,ik)*dtmp1 &
                  &     +  fsr_diff_l(ib,lmta2,ik)*fsr_l_2D(ib,lmta1) &
                  &     +  fsi_diff_l(ib,lmta2,ik)*dtmp2 )
!!$ASASASASAS
                 hsrd(ia,lmt1,lmt2,ispin,i1,i2)=hsrd(ia,lmt1,lmt2,ispin,i1,i2) &
                                  &     + c
                 esrd(ia,lmt1,lmt2,ispin,i1,i2)=esrd(ia,lmt1,lmt2,ispin,i1,i2) &
    ...
    call mpi_allreduce(hsrd,hsrd_mpi,natm*nlmt*nlmt*nspin*3*3,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
    call mpi_allreduce(esrd,esrd_mpi,natm*nlmt*nlmt*nspin*3*3,mpi_double_precision,mpi_sum,MPI_CommGroup,ierr)
    hsrd = hsrd_mpi/dble(nrank_g)
    esrd = esrd_mpi/dble(nrank_g)
...
!!$ASASASASAS
  ...
  subroutine m_Stress_lclchg_diff(nfout)
    ...
    real(kind=DP), pointer, dimension(:)          :: drhodh
...
#endif
    integer :: ibl1,ibl2,nblk
    integer    :: id_sname = -1

    if(modnrm == SKIP) return

    call tstatc0_begin('m_Stress_lclchg_diff ',id_sname)

    do i1=1,3
    do i2=1,3

    allocate(drhodh(nspin))
...
!    do ia = 1, natm
    do ia = ista_atm_ke, iend_atm_ke
       ...
       if(mdvdb == SKIP) cycle
       call calc_phase2_blk(natm,pos,ia,kgp,ngabc_kngp_l,ibl1,ibl2,ista_kngp,iend_kngp,zfcos_tmp,zfsin_tmp)
    ...
    do ia = 1, natm
       ...
       mdvdb = m_PP_include_vanderbilt_pot(it)
       if(mdvdb == SKIP) cycle
       call calc_phase2(natm,pos,ia,kgp,ngabc_kngp_l,ista_kngp,iend_kngp,zfcos_tmp,zfsin_tmp)
