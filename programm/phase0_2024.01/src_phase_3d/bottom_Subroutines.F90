...
!===============================================================================
!$$#ifdef PARA3D
subroutine k_plus_G_vectors_3D(ik,kgp,kg1,knv3,iba,nbase,vk,ngabc,rltv&
     &,qx,qy,qz,vlen)
  use m_Const_Parameters, only : DP, CRDTYP, BUCS
  use m_Parallelization,only       : ista_g1k ,np_g1k
  ...
  integer, intent(in)        :: ngabc(kgp,3)
  real(kind=DP), intent(in)  :: rltv(3,3)
  real(kind=DP), intent(out) :: qx(maxval(np_g1k)),qy(maxval(np_g1k)),qz(maxval(np_g1k)),vlen(maxval(np_g1k))
!  real(kind=DP), intent(out) :: qx(np_g1k(ik)),qy(np_g1k(ik)),qz(np_g1k(ik)),vlen(np_g1k(ik))
...
end subroutine k_plus_G_vectors_3D

subroutine k_plus_G_vectors_m_3D(ik,kgp,kg1,knv3,iba,nbase,vkxyz,ngabc,rltv&
     &,qx,qy,qz)
  use m_Const_Parameters, only : DP, CRDTYP, BUCS
  use m_Parallelization,only       : ista_g1k , iend_g1k, np_g1k
  implicit none
  integer, intent(in)        :: ik, kgp,kg1,knv3,iba(knv3),nbase(kg1,knv3)
  real(kind=DP), intent(in)  :: vkxyz(knv3,3,CRDTYP)
  integer, intent(in)        :: ngabc(kgp,3)
  real(kind=DP), intent(in)  :: rltv(3,3)
  real(kind=DP), intent(out) :: qx(maxval(np_g1k)),qy(maxval(np_g1k)) &
       , qz(maxval(np_g1k))
  integer :: i, ip, iadd
  real(kind=DP) :: ga, gb, gc
  ...
  do i = ista_g1k(ik), iend_g1k(ik)
     ...
     gb = vkxyz(ik,2,BUCS) + real(ngabc(ip,2),kind=DP)
     gc = vkxyz(ik,3,BUCS) + real(ngabc(ip,3),kind=DP)
     qx(iadd)  = rltv(1,1)*ga + rltv(1,2)*gb + rltv(1,3)*gc
     qy(iadd)  = rltv(2,1)*ga + rltv(2,2)*gb + rltv(2,3)*gc
     qz(iadd)  = rltv(3,1)*ga + rltv(3,2)*gb + rltv(3,3)*gc
  end do
end subroutine k_plus_G_vectors_m_3D
