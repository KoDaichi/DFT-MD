...
! $Id: m_SpinOrbit_RadInt.F90 633 2020-12-01 05:11:03Z jkoga $
  ...
  use m_PAW_ChargeDensity,    only : paw_dnr, surface_integral_method, &
       &                             m_PAWCD_set_ae_cd_sphex2_2D, &
       &                             m_PAWCD_ae_cd_sphex2_nonclA, &
       &                             m_PAWCD_set_ps_cd_sphex2_2D, &
...
! ============== 2014/08/11
  ...
  use m_Parallelization,  only : ista_natm, iend_natm, ista_nrc, iend_nrc, &
       &                         mpi_natm_world
  use m_PAW_XC_Potential, only : vxc_ae_k_3D, vxc_ps_k_3D
  use mpi
...
! -----------
    ...
    call calc_total_potential_Y00_3D( ia, it, nrc, vtot_rad_ae, 1 )     ! AE
    ...
    if ( sw_use_rphi_Hsoc_rphi == ON ) then
       ...
       call calc_total_potential_Y00_3D( ia, it, nrc, vtot_rad_ps, 2 )     ! PS
...
!
  ...
  subroutine calc_total_potential_Y00_3D( ia, it, nrc, vtot_rad, ae_or_ps )
    integer, intent(in) :: ia, it, nrc, ae_or_ps
    real(kind=DP), intent(inout) :: vtot_rad(mmesh)

    integer :: ir, ierr, itry, ia_add
    real(kind=DP) :: factor, c1, c2, c3
    real(kind=DP) :: f1, f2, f3, r1, r2, r3
    real(kind=DP) :: ctmp1, ctmp2, rtmp1, rtmp2, cn
    real(kind=DP), allocatable :: vxc_wk(:,:)

    factor = sqrt(PAI4)

    allocate( vxc_wk(mmesh,nspin) );  vxc_wk = 0.0d0

    if ( ae_or_ps == 1 ) then              ! AE potential
       if ( ia >= ista_natm .and. ia <= iend_natm ) then
          ia_add = ia -ista_natm +1
          if ( ista_nrc > 0 ) then
             vxc_wk(ista_nrc:iend_nrc,1:nspin) &
                  &  = vxc_ae_k_3D(ista_nrc:iend_nrc,1:nspin,1,ia_add)
          endif
       endif
       call mpi_allreduce( MPI_IN_PLACE, vxc_wk, mmesh*nspin, &
            &              mpi_double_precision, mpi_sum, mpi_natm_world, ierr )

       Do ir=1, nmesh(it)
          if ( nspin==2 ) then
             c1 = vxc_wk( ir,1 ) + vxc_wk( ir,2 )
             c1 = c1 / 2.0d0
          else
             c1 = vxc_wk( ir,1 )
          endif
       ...
       if ( ia >= ista_natm .and. ia <= iend_natm ) then
          ...
          if ( ista_nrc > 0 ) then
             vxc_wk(ista_nrc:iend_nrc,1:nspin) &
                  &  = vxc_ps_k_3D(ista_nrc:iend_nrc,1:nspin,1,ia_add)
          endif
       endif
       call mpi_allreduce( MPI_IN_PLACE, vxc_wk, mmesh*nspin, &
            &              mpi_double_precision, mpi_sum, mpi_natm_world, ierr )
    ...
    return
  end subroutine calc_total_potential_Y00_3D
...
!      dnr = 1
    ...
    if ( noncol ) then
       ...
       if ( ae_or_ps == 1 ) then
          call m_PAWCD_set_ae_cd_sphex2_2D( ia, nspin, nrmax, dnr, &
               &                         msph, rho_work(1:nrmax,:,:), msphmx_chg, &
               &                         num_isph_chg, isph_chg )
       else if ( ae_or_ps == 2 ) then
          call m_PAWCD_set_ps_cd_sphex2_2D( ia, nspin, nrmax, dnr, &
               &                         msph, rho_work(1:nrmax,:,:), msphmx_chg, &
               &                         num_isph_chg, isph_chg )
       endif
