subroutine alloc_qitg_exx()
  ...
  use m_Control_Parameters, only : nmax_G_hyb,sw_change_axis
  ...
  if(.not.allocated(qitg_exx)) then
    if(sw_change_axis==ON)then
       allocate(qitg_exx(nmax_G_hyb,nqitg,nqmk))
    else
       allocate(qitg_exx(ista_kngp_exx:iend_kngp_exx,nqitg,nqmk))
    endif
  endif
