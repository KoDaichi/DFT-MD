module m_ES_ChargeState
  ...
  use m_PlaneWaveBasisSet, only : igfp_nonpara, ngabc_kngp_l
...
!       if(printable) write(nfout,'(" **warn alf is too small: alf=",d20.10)') alf
       call phase_error_with_msg(nfout,'newldg.eq.kg+1',__LINE__,__FILE__)
    endif
...
!
    ...
    if( ist <= iend ) then
       do in = ist, iend  !for mpi
          Do k=1, 3
             gvec(k) = rltv(k,1) *ngabc_kngp_l(in,1) +rltv(k,2)*ngabc_kngp_l(in,2) &
                  &  + rltv(k,3) *ngabc_kngp_l(in,3)
          End Do
          c1 = 0.0d0
          Do n1=1, 3
             Do n2=1, 3
...
!
    ...
    Do ia=1, natm
       ...
       call calc_phase2( 1, vec_r, 1, kgp, ngabc_kngp_l, ista_kngp, iend_kngp, &
            &            zfcos, zfsin )
       csum = 0.0d0
       do in=ist, iend
...
!
    ...
    call calc_phase2( 1, pos_defect, 1, kgp, ngabc_kngp_l, ista_kngp, iend_kngp, &
         &            zfcos, zfsin )
